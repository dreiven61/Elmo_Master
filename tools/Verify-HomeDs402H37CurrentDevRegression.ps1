...
# Freeze the current CREVIS plus one-drive Home topology before re-running the
# H37 software/source contracts on the current dev tree.
Invoke-Verifier 'tools/Verify-HomeOneAxisImplementation.ps1'
...
Assert-Match $homeSourceBlock 'Ds402HomeState\[105\]\s*:=\s*TO_DINT\(completion\.Data\s+and\s+0xFF\)' 'H37 captures the pre-Home operation mode from 0x6061'
Assert-Match $homeSourceBlock '(?s)sdoIsWrite\s*:=\s*TRUE\s*;\s*sdoIndex\s*:=\s*0x6060\s*;.*?sdoData\s*:=\s*Ds402HomeState\[105\]\$UDINT\s*;' 'H37 restores the captured operation mode'
Assert-Match $homeSourceBlock '(?s)case\s+stage\s+of.*?31:.*?completion\.Data\s+and\s+0xFF\)\s*<>\s*Ds402HomeState\[105\]\$UDINT' 'H37 verifies the captured operation mode after restore'
Assert-Match $homeSourceBlock '(?s)23:.*?statusWord\s+and\s+0x1000.*?Ds402HomeState\[92\]\s*:=\s*24' 'H37 completion advances on drive Homing attained'
Assert-NoMatch $homeSourceBlock 'LMC_DIAG_DS402_HOME_TARGET_REACHED_GRACE_MS|homeCompletionStatusValid|targetReachedGraceExpired' 'H37 completion has no TargetReached grace gate'
...
Assert-Match $diagnostics '(?s)HandleAxisDs402HomeStart\b.*?axisMask\s+and\s+LMC_DIAG_CONFIGURED_PHYSICAL_DRIVE_MASK\)\s*=\s*0\)\s+then\s+detailCode\s*:=\s*4\s*;' 'H37 Start rejects Axis2..4 through the configured physical-drive mask'
...
# The nested activation verifier proves the feature-specific activation set.
