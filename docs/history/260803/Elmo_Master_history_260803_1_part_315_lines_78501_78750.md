...
>
> ```text
> Class View scrolled to class nodes
> ```
>
...
> ```json
> {
>   "code": "{\n  const observation = globalThis.state;\n  const screenshotId = observation.screenshots?.[0]?.id;\n  globalThis.state = null;\n  await sky.click({ window: observation.window, screenshotId, x: 1183, y: 278 });\n  globalThis.state = await sky.get_window_state({ window: observation.window, include_screenshot: true, include_text: false });\n  globalThis.targetWindow = state.window;\n  nodeRepl.write(\"Expanded LMCDiagnosticsService\");\n}",
>   "title": "Diagnostics 클래스 펼치기",
>   "timeout_ms": 30000
> }
> ```
>
> ```text
> Expanded LMCDiagnosticsService
> ```
>
> Image output: image/jpeg
>
