...
> ```json
> {
>   "title": "Inspect Result menu",
>   "code": "globalThis.state = await globalThis.sky.get_window_state({window:globalThis.targetWindow,include_screenshot:true,include_text:true}); nodeRepl.write(globalThis.state.accessibility.tree.split(\"\\n\").filter(x=>/New Input|New Output|Edit Method|Delete Method/.test(x)).join(\"\\n\")); nodeRepl.write(globalThis.state.screenshots.map((s,i)=>({i,id:s.id,w:s.width,h:s.height,ox:s.originX,oy:s.originY})));"
> }
> ```
>
> ```text
> 			102238 메뉴 항목 Edit Method Enter
> 			102239 메뉴 항목 Delete Method Del
> 			102243 메뉴 항목 New Input Variable
> 			102244 메뉴 항목 New Output Variable[
>   { i: 0, id: 'screenshot-0', w: 1450, h: 763, ox: -1880, oy: 41 },
>   { i: 1, id: 'screenshot-1', w: 1440, h: 753, ox: -1875, oy: 46 },
>   { i: 2, id: 'screenshot-2', w: 182, h: 192, ox: -557, oy: 382 },
>   { i: 3, id: 'screenshot-3', w: 182, h: 4, ox: -553, oy: 574 },
>   { i: 4, id: 'screenshot-4', w: 4, h: 188, ox: -375, oy: 386 }
> ]
> ```
>
> Image output: image/jpeg
...
> ```json
> {
>   "title": "Create Result output",
>   "code": "await globalThis.sky.click({window:globalThis.targetWindow,x:75,y:150,screenshotId:globalThis.state.screenshots[2].id}); await globalThis.sky.type_text({window:globalThis.targetWindow,text:\"Result\"}); await globalThis.sky.press_key({window:globalThis.targetWindow,key:\"ENTER\"});"
> }
> ```
>
> ```text
...
> ```json
> {
>   "title": "Verify snapshot signature",
...
>       id: 'screenshot-0',
>       zIndex: 0,
>       url: 'data:image/jpeg;base64,[OMITTED_BASE64 source=Emlo_Master_history_260803_1.md line=18628 chars=9977 sha256=3d34baa568e0957b24c1eb43e1acaa925a0e32e1aeddde2bed3a14da52fe4653]'... 211531 more characters,
>       originX: -1880,
>       originY: 41,
>       width: 1450,
>       height: 763
...
>       id: 'screenshot-1',
>       zIndex: 1,
>       url: 'data:image/jpeg;base64,[OMITTED_BASE64 source=Emlo_Master_history_260803_1.md line=18637 chars=9977 sha256=2743db4c85442c03990bff57c7553f8e898672daf40dd4399065c5f949a343a2]'... 191727 more characters,
>       originX: -1875,
>       originY: 46,
>       width: 1440,
>       height: 753
...
>       '\t\t\t16 창 LMCSdoExecutor Secondary Actions: Raise ID: 65281\n' +
>       '\t\t\t\t17 창 ID: 59648\n' +
>       '\t\t\t\t\t18 창 #pragma usingLtd SigCLib #define LMC_SDO_EXEC_IDLE 0 #define LMC_SDO_EXEC_ARMING 1 #define LMC_SDO_EXEC_RUNNING 2 #define LMC_SDO_EXEC_RESULT_READY 3 #define LMC_SDO_EXEC_ORPHANED 4 #define LMC_SDO_EXEC_QUARANTINED 5 #define LMC_SDO_EXEC_RELEASING 6 #define LMC_SDO_EXEC_VALID 0$UDINT #define LMC_SDO_EXEC_INVALID_VERSION 1 #define LMC_SDO_EXEC_INVALID_STATE 2 #define LMC_SDO_EXEC_INVALID_DIRECTION 3 #define LMC_SDO_EXEC_INVALID_INDEX 4 #define LMC_SDO_EXEC_INVALID_SUBINDEX 5 #define LMC_SDO_EXEC_INVALID_TOKEN 6 #define LMC_SDO_EXEC_INVALID_LENGTH 7 FUNCTION VIRTUAL GLOBAL LMCSdoExecutor::ParaReadWrite::Write VAR_INPUT input (EAX) : DINT; END_VAR VAR_OUTPUT result (EAX) : DINT; END_VAR // The production executor cannot be started through the manual channel. result := ParaReadWrite; END_FUNCTION FUNCTION VIRTUAL GLOBAL LMCSdoExecutor::ParaType::Write VAR_INPUT input (EAX) : '... 13520 more characters,
...
> ```json
> {
>   "title": "Locate Global methods folder",
>   "code": "nodeRepl.write(globalThis.state.accessibility.tree.split(\"\\n\").filter(x=>/selectable\\) Global|CopyTopologyIoSnapshot|Result/.test(x)).join(\"\\n\"));"
> }
> ```
>
> ```text
...
> ```json
> {
>   "title": "Create revision helper",
>   "code": "await globalThis.sky.click({window:globalThis.targetWindow,element_index:71826,mouse_button:\"right\"});"
> }
> ```
>
> ```text
...
> ```json
> {
>   "title": "Inspect Global method menu",
>   "code": "globalThis.state = await globalThis.sky.get_window_state({window:globalThis.targetWindow,include_screenshot:true,include_text:true}); nodeRepl.write(globalThis.state.accessibility.tree.split(\"\\n\").filter(x=>/menu item|메뉴 항목/.test(x)).join(\"\\n\")); nodeRepl.write(globalThis.state.screenshots.map((s,i)=>({i,id:s.id,w:s.width,h:s.height,ox:s.originX,oy:s.originY})));"
> }
> ```
>
> ```text
> 			103782 메뉴 항목 New Method Ins
> 			103783 메뉴 항목 New Std. Methods
> 			103784 메뉴 항목 Sort Methods
> 			103785 메뉴 항목 Paste
> 			103786 메뉴 항목 Copy
> 				103797 메뉴 항목 Macros
> 				103798 메뉴 항목 Layouts
> 				103800 메뉴 항목 Toolbar Options
> 				103802 메뉴 항목 Toolbar Options
