...
>       '\t\t\t\t17 창 ID: 59648\n' +
>       '\t\t\t\t\t18 창 #pragma usingLtd SigCLib #define LMC_SDO_EXEC_IDLE 0 #define LMC_SDO_EXEC_ARMING 1 #define LMC_SDO_EXEC_RUNNING 2 #define LMC_SDO_EXEC_RESULT_READY 3 #define LMC_SDO_EXEC_ORPHANED 4 #define LMC_SDO_EXEC_QUARANTINED 5 #define LMC_SDO_EXEC_RELEASING 6 #de'... 13842 more characters,
...
> ```json
> {
>   "title": "ClassState를 Coupler에 연결",
>   "code": "await globalThis.sky.drag({window:globalThis.targetWindow,from_x:1237,from_y:259,to_x:586,to_y:330});"
> }
> ```
>
> ```text
...
>       id: 'screenshot-0',
>       zIndex: 0,
>       url: 'data:image/jpeg;base64,[OMITTED_BASE64 source=Emlo_Master_history_260803_1.md line=25101 chars=9977 sha256=a429bcb45353367824443e6bdd190064a8ac50f2a6bd1e771c57c677df13f7b4]'... 204871 more characters,
>       originX: -1880,
>       originY: 41,
>       width: 1450,
>       height: 763
...
>       id: 'screenshot-1',
>       zIndex: 1,
>       url: 'data:image/jpeg;base64,[OMITTED_BASE64 source=Emlo_Master_history_260803_1.md line=25110 chars=9977 sha256=93202d42bbcfc72ccf2dc34f90c3c888217fac12f896ed05ca69612aa1991fc0]'... 188811 more characters,
>       originX: -1875,
>       originY: 46,
>       width: 1440,
>       height: 753
...
>       '\t\t\t16 창 LMCSdoExecutor Secondary Actions: Raise ID: 65281\n' +
>       '\t\t\t\t17 창 ID: 59648\n' +
>       '\t\t\t\t\t18 창 #pragma usingLtd SigCLib #define LMC_SDO_EXEC_IDLE 0 #define LMC_SDO_EXEC_ARMING 1 #define LMC_SDO_EXEC_RUNNING 2 #define LMC_SDO_EXEC_RESULT_READY 3 #define LMC_SDO_EXEC_ORPHANED 4 #define LMC_SDO_EXEC_QUARANTINED 5 #define LMC_SDO_EXEC_RELEASING 6 #de'... 13807 more characters,
...
> ```json
> {
>   "title": "LASAL 네트워크 도구 확인",
>   "code": "globalThis.state = await globalThis.sky.get_window_state({window:globalThis.targetWindow,include_screenshot:false,include_text:true}); var lines=(globalThis.state.accessibility?.tree||\"\").split(\"\\n\"); nodeRepl.write(lines.filter(x=>/도구 모음 Net Edit|Toolbar Net Edit|단추 (Select|Zoom|.*Connection|Object|Channel)|메뉴 항목.*Connection/i.test(x)).join(\"\\n\"));"
> }
> ```
>
> ```text
> 			73 도구 모음 Net Edit
> 				203429 단추 Select
> 				203433 메뉴 항목 Online Connection
> 				203434 단추 (disabled) Set Online Connection For Current Project
> ```
>
...
> ```json
> {
>   "title": "Net Edit 도구 전체 확인",
>   "code": "var lines2=(globalThis.state.accessibility?.tree||\"\").split(\"\\n\"); var idx2=lines2.findIndex(x=>x.includes(\"도구 모음 Net Edit\")); nodeRepl.write(lines2.slice(idx2,idx2+30).join(\"\\n\"));"
> }
> ```
>
> ```text
> 			73 도구 모음 Net Edit
> 				203429 단추 Select
> 				203430 메뉴 항목 Toolbar Options
> 			76 도구 모음 Debug
> 				203431 단추 Go online (Alt+F6)
> 				203432 단추 Change Online Settings
> 				203433 메뉴 항목 Online Connection
> 				203434 단추 (disabled) Set Online Connection For Current Project
> 				203435 단추 (disabled) Download (F6)
> 				203436 단추 Build Changes, Online, Download and Run (CTRL+ALT+F6)
> 				203437 단추 (disabled) Download Module on the Fly
> 				203438 단추 (disabled) Save Project on PLC
> 				203439 단추 (disabled) Start (F7)
> 				203440 단추 (disabled) Reset (F8)
> 				203441 단추 (disabled) Toggle breakpoint (F4)
> 				203442 단추 (disabled) Create condition breakpoint
> 				203443 메뉴 항목 Toolbar Options
> 			90 도구 모음 Build
> 				203444 메뉴 항목 Target Architecture
> 				203445 단추 Build changes (F9)
> 				203446 단추 Rebuild project (Strg+F9)
> 				203447 단추 (disabled) Cancel building (Ctrl+Break)
> 				203448 단추 Link project
> 			96 도구 모음 Standard
> 				203449 단추 New project (Strg+N)
> 				203450 단추 Open a file (Strg+Shift+O)
> 				203451 단추 Close active document (Strg+F4)
> 				203452 단추 (disabled) Save file (Strg+S)
> 				203453 단추 Open project (Strg+O)
> 				203454 단추 (disabled) Save project changes (Strg+Shift+S)
> ```
>
> </details>
>
...
> {
>   "title": "드래그 입력 API 확인",
>   "code": "var ddrag=await globalThis.sky.documentation({query:\"drag mouse down move up\",include:\"index,examples,notes\",max_chars:12000}); nodeRepl.write(ddrag);"
> }
