...
>   "code": "globalThis.apps = await sky.list_apps();\nnodeRepl.write(JSON.stringify(apps.map(a => ({id:a.id,name:a.name,windows:a.windows.map(w=>({id:w.id,app:w.app,title:w.title}))})).filter(a => /lasal/i.test((a.name||\"\")+\" \"+(a.id||\"\"))), null, 2));"
> }
> ```
>
> ```text
...
>         "title": "LASAL Motion Control API Example v0.9.1.0 [LIVE Axis qualification / qualified Axis1 UI24 SDO Write]"
>       }
>     ]
>   },
>   {
