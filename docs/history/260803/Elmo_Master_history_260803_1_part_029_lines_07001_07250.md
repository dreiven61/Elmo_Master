...
>   "code": "\nglobalThis.targetApp = globalThis.apps.find(a => a.id === \"{7C5A40EF-A0FB-4BFC-874A-C0F2E0B9FA8E}\\\\Sigmatek\\\\Lasal\\\\Class2\\\\Bin\\\\Lasal2.exe\");\nif (!globalThis.targetApp || globalThis.targetApp.windows.length !== 1) throw new Error(\"Expected exactly one LASAL window\");\nglobalThis.returnedWindow = globalThis.targetApp.windows[0];\nglobalThis.targetWindow = await sky.get_window({id:globalThis.returnedWindow.id, app:globalThis.returnedWindow.app});\nawait sky.activate_window({window:globalThis.targetWindow});\nglobalThis.state = await sky.get_window_state({window:globalThis.targetWindow,include_screenshot:true,include_text:true});\nglobalThis.targetWindow = globalThis.state.window;\nnodeRepl.write(String(globalThis.state.accessibility?.tree || globalThis.state.accessibility?.document_text || \"\"));\n"
> }
> ```
>
> ```text
...
> 			56 도구 모음 Script
> 			57 도구 모음 Edit
> 				58 단추 Toggle bookmark
> 				59 단추 (disabled) Previous bookmark
> 				60 단추 (disabled) Next bookmark
> 				61 단추 (disabled) Delete all bookmarks
> 				62 단추 (disabled) Previous bookmark in this file
> 				63 단추 (disabled) Next bookmark in this file
> 				64 단추 Comment selected text (Ctrl+Shift+C)
> 				65 단추 Remove comment (Ctrl+Shift+X)
> 				66 단추 Unindent (Shift+Tab)
> 				67 단추 Indent (Tab)
...
> 				75 단추 DiasDrive
> 				76 단추 PLC Diagnosis
> 				77 단추 Hardware Editor
> 				78 단추 Graphical Hardware Editor
> 				79 단추 Connection Manager
> 				80 단추 Task Configuration
> 			81 도구 모음 Net Edit
> 				82 단추 (disabled) Select
> 				83 단추 (disabled) Move view
> 				84 단추 (disabled) Insert comment
> 				85 단추 (disabled) Zoom(+/-)
> 				86 단추 (disabled) Zoom to all
> 				87 단추 (disabled) Zoom selection
> 			88 도구 모음 Debug
> 				89 단추 Go online (Alt+F6)
> 				90 단추 Change Online Settings
> 				91 메뉴 항목 Online Connection
> 				92 단추 (disabled) Set Online Connection For Current Project
> 				93 단추 (disabled) Download (F6)
> 				94 단추 Build Changes, Online, Download and Run (CTRL+ALT+F6)
> 				95 단추 (disabled) Download Module on the Fly
> 				96 단추 (disabled) Save Project on PLC
> 				97 단추 (disabled) Start (F7)
> 				98 단추 (disabled) Reset (F8)
> 				99 단추 Toggle breakpoint (F4)
> 				100 단추 Create condition breakpoint
> 				101 단추 (disabled) Remove all breakpoint(s) (Shift+F4)
> 				102 단추 AWL trigger on/off
> 				103 단추 (disabled) Fix AWL trigger to current instruction
> 				104 단추 Activate/Deactivate Live View
> 				105 단추 Activate/Deactivate Auto Watch
> 				106 단추 (disabled) Goto instruction pointer
> 				107 단추 (disabled) Step into (F5)
> 				108 단추 (disabled) Step over (Alt+F5)
> 				109 단추 (disabled) Step out (Shift+F5)
> 				110 단추 (disabled) Set instruction pointer
> 			111 도구 모음 Build
> 				112 메뉴 항목 Target Architecture
> 				113 단추 Build changes (F9)
> 				114 단추 Rebuild project (Strg+F9)
> 				115 단추 (disabled) Cancel building (Ctrl+Break)
> 				116 단추 Link project
> 			117 도구 모음 Standard
> 				118 단추 New project (Strg+N)
> 				119 단추 Open a file (Strg+Shift+O)
> 				120 단추 Close active document (Strg+F4)
> 				121 단추 (disabled) Save file (Strg+S)
> 				122 단추 Open project (Strg+O)
> 				123 단추 (disabled) Save project changes (Strg+Shift+S)
> 				124 단추 Close project
> 				125 단추 Print
> 				126 단추 Cut (Strg+X)
> 				127 단추 Copy (Strg+C)
> 				128 단추 Paste (Strg+V)
> 				129 메뉴 항목 (disabled) Undo (Strg+Z)
> 				130 메뉴 항목 (disabled) Redo (Strg+Y)
> 				131 단추 Navigate Backward (Alt+Left)
> 				132 단추 (disabled) Navigate Forward (Alt +Right)
> 			133 메뉴 모음 Menu Bar
> 				134 메뉴 항목 FILE
> 				135 메뉴 항목 EDIT
> 				136 메뉴 항목 VIEW
...
> 			166 탭 항목 (selectable) Python Script
> 			167 탭 항목 (selectable) Debugger
> 			168 탭 항목 (selectable) Output
> 			169 단추 Close
