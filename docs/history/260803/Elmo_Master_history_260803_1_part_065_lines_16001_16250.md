...
> ```json
> {
>   "title": "OutputPreviousValue 타입 재편집",
...
> 		70287 편집
> 		70288 LIST (truncated: child_limit, omitted 604 children)
> 		1 창 작업 영역 ID: 59648
...
> {
>   "title": "새 메서드 메뉴 열기",
>   "code": "await globalThis.sky.click({window:globalThis.targetWindow,element_index:17898,mouse_button:\"right\"});"
> }
> ```
>
> ```text
...
> {
>   "title": "메서드 메뉴 확인",
>   "code": "globalThis.state = await globalThis.sky.get_window_state({window:globalThis.targetWindow,include_screenshot:true,include_text:true}); nodeRepl.write(globalThis.state.accessibility.tree.split(\"\\n\").filter(x=>x.includes(\"메뉴 항목 New\")).join(\"\\n\"));"
> }
> ```
>
> Image output: image/jpeg
...
> {
>   "title": "기존 메서드 목록 펼치기",
>   "code": "await globalThis.sky.click({window:globalThis.targetWindow,x:1199,y:239,screenshotId:globalThis.state.screenshots[1].id});"
> }
> ```
>
> ```text
...
> {
>   "title": "기존 메서드 구조 확인",
>   "code": "globalThis.state = await globalThis.sky.get_window_state({window:globalThis.targetWindow,include_screenshot:true,include_text:true}); nodeRepl.write(globalThis.state.accessibility.tree.split(\"\\n\").filter(x=>x.includes(\"CopySnapshot\")||x.includes(\"RtWork\")||/\\bMethods\\b/.test(x)).join(\"\\n\"));"
> }
> ```
>
> ```text
...
> ```json
> {
>   "title": "Global 메서드 목록 펼치기",
>   "code": "await globalThis.sky.click({window:globalThis.targetWindow,x:1215,y:257,screenshotId:globalThis.state.screenshots[1].id});"
> }
> ```
>
> ```text
...
> ```json
> {
>   "title": "Global 메서드 구조 읽기",
>   "code": "globalThis.state = await globalThis.sky.get_window_state({window:globalThis.targetWindow,include_screenshot:true,include_text:true}); nodeRepl.write(globalThis.state.accessibility.tree.split(\"\\n\").filter(x=>x.includes(\"CopySnapshot\")||x.includes(\"RtWork\")||x.includes(\"Global\")).join(\"\\n\"));"
> }
> ```
>
> ```text
...
> ```json
> {
>   "title": "Global 메서드 메뉴 확인",
>   "code": "globalThis.state = await globalThis.sky.get_window_state({window:globalThis.targetWindow,include_screenshot:true,include_text:true}); nodeRepl.write(globalThis.state.accessibility.tree.split(\"\\n\").filter(x=>x.includes(\"메뉴 항목\")).slice(0,12).join(\"\\n\"));"
> }
> ```
>
> ```text
> 			74452 메뉴 항목 New Method Ins
> 			74453 메뉴 항목 New Std. Methods
> 			74454 메뉴 항목 Sort Methods
> 			74455 메뉴 항목 Paste
> 			74456 메뉴 항목 Copy
> 				74467 메뉴 항목 Macros
> 				74468 메뉴 항목 Layouts
> 				74470 메뉴 항목 Toolbar Options
> 				74472 메뉴 항목 Toolbar Options
> 				74475 메뉴 항목 Online Connection
> 				74485 메뉴 항목 Toolbar Options
> 				74486 메뉴 항목 Target Architecture
> ```
>
> Image output: image/jpeg
>
...
> ```json
> {
>   "title": "CopyTopologyIoSnapshot 메서드 생성",
>   "code": "await globalThis.sky.click({window:globalThis.targetWindow,x:1322,y:271,screenshotId:globalThis.state.screenshots[1].id});"
> }
> ```
>
> ```text
...
> ```json
> {
>   "title": "CopyTopologyIoSnapshot 이름 입력",
>   "code": "await globalThis.sky.type_text({window:globalThis.targetWindow,text:\"CopyTopologyIoSnapshot\"});"
> }
> ```
>
> ```text
