...
> ```json
> {
>   "title": "LASAL Class View 선택",
>   "code": "var obsTreeFocus=globalThis.state; var sidTreeFocus=obsTreeFocus.screenshots[0].id; globalThis.state=null; try{await sky.click({window:obsTreeFocus.window,screenshotId:sidTreeFocus,x:820,y:235}); globalThis.state=await sky.get_window_state({window:obsTreeFocus.window,include_screenshot:true,include_text:true});}catch(error){throw new Error('Tree focus outcome unknown: '+String(error));} globalThis.targetWindow=state.window; nodeRepl.write(String(state.accessibility?.focused_element||state.accessibility?.tree||''));",
>   "timeout_ms": 30000
> }
> ```
>
...
>   "code": "var obsTreeScroll=globalThis.state; globalThis.state=null; try{await sky.scroll({window:obsTreeScroll.window,x:820,y:235,scrollX:0,scrollY:500}); globalThis.state=await sky.get_window_state({window:obsTreeScroll.window,include_screenshot:true,include_text:true});}catch(error){throw new Error('Tree scroll outcome unknown: '+String(error));} globalThis.targetWindow=state.window; nodeRepl.write(String(state.accessibility?.tree||''));",
>   "timeout_ms": 30000
> }
> ```
>
...
> 			13 도구 모음 Edit
> 				14483 단추 Toggle bookmark
> 				14484 단추 (disabled) Previous bookmark
> 				14485 단추 (disabled) Next bookmark
> 				14486 단추 (disabled) Delete all bookmarks
> 				14487 단추 (disabled) Previous bookmark in this file
> 				14488 단추 (disabled) Next bookmark in this file
> 				14489 단추 (disabled) Comment selected text (Ctrl+Shift+C)
> 				14490 단추 (disabled) Remove comment (Ctrl+Shift+X)
> 				14491 단추 (disabled) Unindent (Shift+Tab)
> 				14492 단추 (disabled) Indent (Tab)
> 			24 도구 모음 Macros Manager
> 				14493 메뉴 항목 Macros
> 			26 도구 모음 Layout Manager
> 				14494 메뉴 항목 Layouts
> 			28 도구 모음 Toolbox
> 				14495 단추 DataAnalyzer
> 				14496 메뉴 항목 Toolbar Options
> 			31 도구 모음 Net Edit
> 				14497 단추 (disabled) Select
> 				14498 메뉴 항목 Toolbar Options
> 			34 도구 모음 Debug
> 				14499 단추 Go online (Alt+F6)
> 				14500 메뉴 항목 Toolbar Options
> 			37 도구 모음 Build
> 				14501 메뉴 항목 Target Architecture
> 				14502 메뉴 항목 Toolbar Options
> 			40 도구 모음 Standard
> 				14503 단추 New project (Strg+N)
> 				14504 단추 Open a file (Strg+Shift+O)
> 				14505 단추 Close active document (Strg+F4)
> 				14506 단추 (disabled) Save file (Strg+S)
> 				14507 단추 Open project (Strg+O)
> 				14508 단추 (disabled) Save project changes (Strg+Shift+S)
> 				14509 단추 Close project
> 				14510 단추 Print
> 				14511 단추 Cut (Strg+X)
> 				14512 단추 Copy (Strg+C)
> 				14513 단추 Paste (Strg+V)
> 				14514 메뉴 항목 (disabled) Undo (Strg+Z)
> 				14515 메뉴 항목 (disabled) Redo (Strg+Y)
> 				14516 메뉴 항목 Toolbar Options
> 			55 메뉴 모음 Menu Bar
> 				14517 메뉴 항목 FILE
> 				14518 메뉴 항목 EDIT
> 				14519 메뉴 항목 VIEW
...
> 			14528 탭 항목 (selectable) Python Script
> 			14529 탭 항목 (selectable) Debugger
> 			14530 탭 항목 (selectable) Output
> 			80 단추 Close
...
> 			14532 탭 항목 (selectable) Lib
> 			14533 탭 항목 (selectable) File
> 			14534 탭 항목 (selectable) Class
> 			87 단추 Close
...
> 					92 단추
> 					93 단추
> 			14535 탭 항목 (selectable) Properties
> 			95 단추 Close
>
...
> ```json
> {
>   "title": "LMCDiagnosticsService 메뉴 확인",
>   "code": "var obsDiagContext=globalThis.state; var sidDiagContext=obsDiagContext.screenshots[0].id; globalThis.state=null; try{await sky.click({window:obsDiagContext.window,screenshotId:sidDiagContext,x:815,y:157,mouse_button:'right'}); globalThis.state=await sky.get_window_state({window:obsDiagContext.window,include_screenshot:true,include_text:true});}catch(error){throw new Error('Diagnostics context menu outcome unknown: '+String(error));} globalThis.targetWindow=state.window; nodeRepl.write(String(state.accessibility?.tree||''));",
>   "timeout_ms": 30000
> }
> ```
>
