...
>   "code": "var obsOpenProject = globalThis.state; globalThis.state = null; try { await sky.press_key({window: obsOpenProject.window, key: 'Control_L+o'}); globalThis.state = await sky.get_window_state({window: obsOpenProject.window, include_screenshot: true, include_text: true}); } catch (error) { throw new Error('Open project action outcome unknown', {cause:error}); } globalThis.targetWindow = state.window; nodeRepl.write(String(state.accessibility?.tree || state.accessibility?.document_text || ''));",
>   "timeout_ms": 30000
> }
> ```
>
> ```text
> Open project action outcome unknown
> ```
>
> MCP tool call
>
...
>   "code": "globalThis.apps2 = await sky.list_apps(); var lasalMatches2 = apps2.filter(a => /Lasal2\\.exe/i.test(a.id) || /LASAL Class 2/i.test(a.displayName||'')); nodeRepl.write(JSON.stringify(lasalMatches2, null, 2));",
>   "timeout_ms": 30000
> }
> ```
>
