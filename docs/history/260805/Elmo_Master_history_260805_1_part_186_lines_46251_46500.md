> 				6371 메뉴 항목 (disabled) Redo (Strg+Y)
> 				6372 메뉴 항목 Toolbar Options
> 			55 메뉴 모음 Menu Bar
> 				6373 메뉴 항목 FILE
> 				6374 메뉴 항목 EDIT
> 				6375 메뉴 항목 VIEW
...
> 			6384 탭 항목 (selectable) Python Script
> 			6385 탭 항목 (selectable) Debugger
> 			6386 탭 항목 (selectable) Output
> 			80 단추 Close
...
> 			6387 탭 항목 (selectable) Lib
> 			6388 탭 항목 (selectable) File
> 			6389 탭 항목 (selectable) Class
> 			6390 탭 항목 (selectable) Global
> 			88 단추 Close
...
> 					93 단추
> 					94 단추
> 			6391 탭 항목 (selectable) Properties
> 			96 단추 Close
>
...
>   "code": "var obsBuildMenu2=state; state=null; await sky.click({window:obsBuildMenu2.window,element_index:6377}); state=await sky.get_window_state({window:obsBuildMenu2.window,include_screenshot:true,include_text:true}); targetWindow=state.window; nodeRepl.write(String(state.accessibility?.tree||''));"
> }
> ```
>
> ```text
> element 6377 is not available in cached app state for Lasal2.exe
> ```
>
> MCP tool call
>
...
> ```json
> {
>   "title": "Observe LASAL build menu bar",
>   "code": "state=await sky.get_window_state({window:targetWindow,include_screenshot:true,include_text:false}); targetWindow=state.window; nodeRepl.write('Ready for Build menu click');"
> }
> ```
>
> ```text
> Ready for Build menu click
> ```
>
...
> {
>   "title": "Open LASAL build menu",
>   "code": "var obsBuildMenu3=state; var sidBuildMenu3=obsBuildMenu3.screenshots[0].id; state=null; await sky.click({window:obsBuildMenu3.window,screenshotId:sidBuildMenu3,x:245,y:42}); state=await sky.get_window_state({window:obsBuildMenu3.window,include_screenshot:true,include_text:true}); targetWindow=state.window; nodeRepl.write(String(state.accessibility?.tree||''));"
> }
> ```
>
> ```text
...
> 			13 도구 모음 Edit
> 				7717 단추 Toggle bookmark
> 				7718 단추 (disabled) Previous bookmark
> 				7719 단추 (disabled) Next bookmark
> 				7720 단추 (disabled) Delete all bookmarks
> 				7721 단추 (disabled) Previous bookmark in this file
> 				7722 단추 (disabled) Next bookmark in this file
> 				7723 단추 Comment selected text (Ctrl+Shift+C)
> 				7724 단추 Remove comment (Ctrl+Shift+X)
> 				7725 단추 Unindent (Shift+Tab)
> 				7726 단추 Indent (Tab)
...
> 				7728 메뉴 항목 Layouts
> 			28 도구 모음 Toolbox
> 				7729 단추 DataAnalyzer
> 				7730 메뉴 항목 Toolbar Options
> 			31 도구 모음 Net Edit
> 				7731 단추 (disabled) Select
> 				7732 메뉴 항목 Toolbar Options
> 			34 도구 모음 Debug
> 				7733 단추 Go online (Alt+F6)
> 				7734 메뉴 항목 Toolbar Options
> 			37 도구 모음 Build
> 				7735 메뉴 항목 Target Architecture
> 				7736 메뉴 항목 Toolbar Options
> 			40 도구 모음 Standard
> 				7737 단추 New project (Strg+N)
> 				7738 단추 Open a file (Strg+Shift+O)
> 				7739 단추 Close active document (Strg+F4)
> 				7740 단추 (disabled) Save file (Strg+S)
> 				7741 단추 Open project (Strg+O)
> 				7742 단추 (disabled) Save project changes (Strg+Shift+S)
> 				7743 단추 Close project
> 				7744 단추 Print
> 				7745 단추 Cut (Strg+X)
> 				7746 단추 Copy (Strg+C)
> 				7747 단추 (disabled) Paste (Strg+V)
> 				7748 메뉴 항목 (disabled) Undo (Strg+Z)
> 				7749 메뉴 항목 (disabled) Redo (Strg+Y)
> 				7750 메뉴 항목 Toolbar Options
> 			55 메뉴 모음 Menu Bar
> 				7751 메뉴 항목 FILE
> 				7752 메뉴 항목 EDIT
> 				7753 메뉴 항목 VIEW
