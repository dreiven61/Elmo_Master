...
> 			28240 탭 항목 (selectable) Python Script
> 			28241 탭 항목 (selectable) Debugger
> 			28242 탭 항목 (selectable) Output
> 			80 단추 Close
...
> 			28243 탭 항목 (selectable) Lib
> 			28244 탭 항목 (selectable) File
> 			28245 탭 항목 (selectable) Class
> 			28246 탭 항목 (selectable) Global
> 			88 단추 Close
...
> 					28247 custom Name
> 					28248 custom Revision
> 					28249 custom GUID
> 					28250 custom Task Settings
> 					28251 custom Sigmatek
> 					28252 custom OSInterface
> 					28253 custom IconPath
> 					28254 custom SharedCommandTable
> 					28255 custom Objectsize
> 					28256 custom Singleton
> 					28257 custom Hidden
> 					28258 custom Deprecated
> 					28259 custom GCCOptions
> 					28260 custom Comment
> 					28261 custom Filename
> 				92 도구 모음 ID: 59392
> 					93 단추
> 					94 단추
> 			28262 탭 항목 (selectable) Properties
> 			96 단추 Close
>
...
> ```json
> {
>   "title": "Zero-home helper 구현 열기",
>   "code": "{const observation=globalThis.state;globalThis.state=null;await sky.press_key({window:observation.window,key:'Escape'});globalThis.state=await sky.get_window_state({window:observation.window,include_screenshot:true,include_text:false});globalThis.targetWindow=state.window;nodeRepl.write('Method menu closed')}"
> }
> ```
>
> ```text
...
> ```json
> {
>   "title": "Zero-home 구현 위치 확인",
>   "code": "{const observation=globalThis.state;const screenshotId=observation?.screenshots?.[0]?.id;if(screenshotId==null)throw new Error('No screenshot id');globalThis.state=null;await sky.click({window:observation.window,screenshotId,x:850,y:157,click_count:2});globalThis.state=await sky.get_window_state({window:observation.window,include_screenshot:true,include_text:true});globalThis.targetWindow=state.window;nodeRepl.write(String(state.accessibility?.tree||state.accessibility?.document_text||''))}"
> }
> ```
>
> ```text
...
> 			13 도구 모음 Edit
> 				29963 단추 (disabled) Toggle bookmark
> 				29964 단추 (disabled) Previous bookmark
> 				29965 단추 (disabled) Next bookmark
> 				29966 단추 (disabled) Delete all bookmarks
> 				29967 단추 (disabled) Previous bookmark in this file
> 				29968 단추 (disabled) Next bookmark in this file
> 				29969 단추 (disabled) Comment selected text (Ctrl+Shift+C)
> 				29970 단추 (disabled) Remove comment (Ctrl+Shift+X)
> 				29971 단추 (disabled) Unindent (Shift+Tab)
