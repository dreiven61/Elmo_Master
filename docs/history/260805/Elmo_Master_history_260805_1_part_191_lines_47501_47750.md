> 			14089 확인란 Exclude Sigmatek classes ID: 1694
> 			14090 확인란 Show in Find Results 2 ID: 1428
> 			14091 확인란 Show Method Name ID: 1840
> 			14092 텍스트 Filename/Extension:
> 			14093 콤보 상자 Filename/Extension: ID: 1011
> 				14094 편집 Filename/Extension: ID: 1001
> 				14095 단추 열기 ID: DropDown
> 			14096 텍스트 Find in Folder:
> 			14097 콤보 상자 (disabled) Find in Folder: ID: 1012
> 				14098 편집 (disabled) Find in Folder: ID: 1001
> 				14099 단추 열기 ID: DropDown
> 			14100 단추 ... ID: 1013
> 			14101 확인란 (disabled) Include subfolders ID: 1017
> 			14102 단추 OK ID: 1
> 			14103 단추 (disabled) Break ID: 1159
> 			14104 단추 Cancel ID: 2
> 			14105 제목 표시줄
> 				14106 단추 닫기
>
...
> ```json
> {
>   "title": "Focus LASAL search field",
>   "code": "var obsFind2=state; var sidFind2=obsFind2.screenshots[0].id; state=null; await sky.click({window:obsFind2.window,screenshotId:sidFind2,x:444,y:134}); state=await sky.get_window_state({window:obsFind2.window,include_screenshot:true,include_text:true}); targetWindow=state.window; nodeRepl.write(String(state.accessibility?.focused_element||''));"
> }
> ```
>
> ```text
...
> ```json
> {
>   "title": "Select LASAL search text",
>   "code": "var obsFind3=state; state=null; await sky.press_key({window:obsFind3.window,key:'Control_L+a'}); state=await sky.get_window_state({window:obsFind3.window,include_screenshot:true,include_text:true}); targetWindow=state.window; nodeRepl.write(String(state.accessibility?.focused_element||''));"
> }
> ```
>
> ```text
...
> ```json
> {
>   "title": "Enter LASAL search marker",
>   "code": "var obsFind4=state; state=null; await sky.type_text({window:obsFind4.window,text:'LMC_OWNER_ORDINARY_CLASSIFIER_BEGIN'}); state=await sky.get_window_state({window:obsFind4.window,include_screenshot:true,include_text:true}); targetWindow=state.window; nodeRepl.write(String(state.accessibility?.focused_element||''));"
> }
> ```
>
> ```text
...
> ```json
> {
>   "title": "Run LASAL marker search",
>   "code": "var obsFind5=state; state=null; await sky.press_key({window:obsFind5.window,key:'Return'}); state=await sky.get_window_state({window:obsFind5.window,include_screenshot:true,include_text:true}); targetWindow=state.window; nodeRepl.write(String(state.accessibility?.tree||''));"
> }
> ```
>
> ```text
...
> 			13 도구 모음 Edit
> 				14319 단추 Toggle bookmark
> 				14320 단추 (disabled) Previous bookmark
> 				14321 단추 (disabled) Next bookmark
> 				14322 단추 (disabled) Delete all bookmarks
> 				14323 단추 (disabled) Previous bookmark in this file
> 				14324 단추 (disabled) Next bookmark in this file
> 				14325 단추 Comment selected text (Ctrl+Shift+C)
> 				14326 단추 Remove comment (Ctrl+Shift+X)
> 				14327 단추 Unindent (Shift+Tab)
> 				14328 단추 Indent (Tab)
...
> 				14330 메뉴 항목 Layouts
> 			28 도구 모음 Toolbox
> 				14331 단추 DataAnalyzer
> 				14332 메뉴 항목 Toolbar Options
> 			31 도구 모음 Net Edit
> 				14333 단추 (disabled) Select
> 				14334 메뉴 항목 Toolbar Options
> 			34 도구 모음 Debug
> 				14335 단추 Go online (Alt+F6)
> 				14336 메뉴 항목 Toolbar Options
> 			37 도구 모음 Build
> 				14337 메뉴 항목 Target Architecture
> 				14338 메뉴 항목 Toolbar Options
> 			40 도구 모음 Standard
> 				14339 단추 New project (Strg+N)
> 				14340 단추 Open a file (Strg+Shift+O)
> 				14341 단추 Close active document (Strg+F4)
> 				14342 단추 (disabled) Save file (Strg+S)
> 				14343 단추 Open project (Strg+O)
> 				14344 단추 (disabled) Save project changes (Strg+Shift+S)
> 				14345 단추 Close project
> 				14346 단추 Print
> 				14347 단추 Cut (Strg+X)
> 				14348 단추 Copy (Strg+C)
> 				14349 단추 (disabled) Paste (Strg+V)
> 				14350 메뉴 항목 (disabled) Undo (Strg+Z)
> 				14351 메뉴 항목 (disabled) Redo (Strg+Y)
> 				14352 메뉴 항목 Toolbar Options
> 			55 메뉴 모음 Menu Bar
> 				14353 메뉴 항목 FILE
> 				14354 메뉴 항목 EDIT
> 				14355 메뉴 항목 VIEW
...
> 				71 창 ID: 1200
> 					14364 LIST ID: 1205
> 						14365 스크롤 막대 세로 ID: NonClientVerticalScrollBar
> 							14366 단추 위쪽 스크롤 화살표 ID: UpButton
> 							14367 단추 페이지 위로 ID: UpPageButton
> 							14368 위치 조정 위치 ID: ScrollbarThumb
> 							14369 단추 아래쪽 스크롤 화살표 ID: DownButton
> 						14370 목록 항목 (selectable)
> 						14371 목록 항목 (selectable)
> 						14372 목록 항목 (selectable)
> 						14373 목록 항목 (selectable)
> 						14374 목록 항목 (selectable)
> 						14375 목록 항목 (selectable)
> 						14376 목록 항목 (selectable)
> 						14377 목록 항목 (selectable)
> 						14378 목록 항목 (selectable)
> 						14379 목록 항목 (selectable)
> 						14380 목록 항목 (selectable)
> 						14381 목록 항목 (selectable)
