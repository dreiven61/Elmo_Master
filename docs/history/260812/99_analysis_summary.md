...
2. 최신 실제 장애는 PC의 UDP port 잔류가 아니라, 새 TCP 연결도
   ...
   경계인지 확정할 수는 없다.
3. 이를 처리하는 **exact `-8` owner-loss retirement**가 두 LASAL `.st` 파일에 구현됐고,
   `bbe8a8d`는 기존 `RpcCallbackLastDisarmResult`를 경계-correlated Watch latch로 보강했다.
4. Predecessor `e3c9365`의 두 `.st` 변경은 approved `24402...` baseline을 배치한 격리 worktree에서 incremental
...
7. `bbe8a8d` 후속 latch 계약은 PS5.1/PS7 `311/311` self-test를 통과했지만 LASAL build와
   sanctioned generated artifact가 없다.
8. **PLC download 및 실제 Connect → Close → Connect 검증은 수행하지 않았다.** 따라서
   실장비 해결로 판정할 수 없다.
...
reconnect를 확인한다. Full Distribution과 predecessor `e3c9365` compile/link는 이미 완료했으므로
반복 대상이 아니다. Latest `bbe8a8d`는 source-bound artifact transition 뒤 같은 exact source로
검증해야 한다.
...
- Predecessor `e3c9365` owner-loss source의 격리 Build는 새 `5337...` artifact를 만들었고 Gate D target
...
검증한 결과다. Main의 사용자 `Classes.lcb`와 untracked artifact는 수정하지 않았다.

| 항목 | 현재 확인값 |
|---|---|
| owner-loss predecessor baseline | `e3c9365b953567b332d6f34f167044d98ef549eb` |
| owner-loss observability implementation | `bbe8a8dd53de0b1af845d0dd088df9c832c4bee6` |
| staged 변경 | 0 |
| handoff commit 뒤 보존할 tracked drift | 1개: 사용자 `Classes.lcb` |
| LASAL2 process | user-owned `Lasal_0094` WTR session PID 41504; 건드리지 않음 |
...
| 최신 owner-loss source | `bbe8a8d`에 commit됨 |
...
산출물은 `docs/history/260812/`에만 만들었다. Predecessor owner-loss/release 수정은
`e3c9365`, 후속 Watch latch/source verifier는 `bbe8a8d`에 분리했다.

현재 핵심 working-tree source SHA-256은 다음과 같다.

| 파일 | SHA-256 |
|---|---|
| `TCPMotionInterface.st` | `50D81625CCE255F91A6A617356220F7139B5F249C6048E2A5FCDA914728DAB98` |
| `LMCUdpCallbackSender.st` | `619EAF7D33208135D93D44FD4703FB385EA1115D1D1352825D6F055349C3C296` |
| `Verify-LasalUdpCallbackContract.ps1` | `BD7CA001EA29A3BA1DA1E68552F72048165EFC63710F19D1A8B3ECA91D04AAC6` |
| `Verify-LasalContract.ps1` | `C7BF4793A36FD3769E7468A6EEB5E96918A881127A89862A9FD7A6F882D635E0` |

## 증거 수준

| 대상 | 상태 | 정확한 해석 |
|---|---|---|
| owner-loss ST 구현 | `e3c9365` + `bbe8a8d` | exact `-8` 두 경로와 existing-field Watch latch를 source/static으로 확인 |
| UDP static verifier | PS5.1/PS7 `311/311` | 두 host 장시간 suite PASS; PLC 증거 아님 |
| strict Auto parser | `12/12` positive, `15/15` negative | 모순 state/prefix/approval tuple을 fail-closed 거부 |
| SourceOnly | predecessor `e3c9365` PS5.1/PS7 PASS | `bbe8a8d` current snapshot은 user WTR LASAL session 때문에 강제하지 않음 |
...
| LASAL build — predecessor `e3c9365` 두 ST | compile/link PASS | `0 errors / 24 warnings`, 두 ST compile; `bbe8a8d` follow-up build는 없음 |
...
| PLC download — latest `bbe8a8d` | 미수행 | PLC에 최신 수정이 들어갔다고 볼 수 없음 |
...
`bbe8a8d`는 각 허용 owner-loss 경계에서 latch를 `1`로 reset하고, 최초 exact `-8`, sentinel
`0/1`, confirmation `0/1`이 모두 성립해 tuple이 지워진 뒤 `RpcCallbackLastDisarmResult=-8`을
다시 남긴다. 미등록 helper no-op `1`은 이 값을 덮지 않는다. Sentinel 실패는 실제 결과,
pre-sentinel sender 소실은 `-9`, negative confirmation은 그 결과를 남긴다.

그러나 final `-8` 하나만으로 성공을 판정할 수 없다. 최초 mismatch와 confirmation mismatch도
`-8`을 남길 수 있다. Accepted-owner 또는 definitive current-socket-disconnect 경계, old
TCP/sender tuple과 QueueDepth clear 또는 fresh current-epoch tuple, 다음 callback 등록/수신을
before/after Watch와 pcap으로 묶어야 exact owner-loss runtime proof다. 경계 시작의 `1`은 transient
reset일 뿐이며 다음 허용 경계는 이전 latch를 덮을 수 있다.
...
5. `bbe8a8d`의 existing-field latch를 사용하되 사건 전·후 latch, socket, TCP/sender tuple,
   QueueDepth를 한 snapshot sequence로 캡처한다. `-8` 단독 PASS는 금지한다.
...
> exact `-8` stale callback owner-loss 정리와 clean full Distribution은 `e3c9365`, 기존-field
> Watch latch와 dual-host `311/311` 정적 회귀는 `bbe8a8d`에 고정했다. 격리 C78/ARM
> incremental Build는 predecessor 두 ST compile/link를
> `0 errors / 24 warnings`로 통과했다. 그러나 post-build `Classes.lcb=5337...`는 Gate D target
> boundary drift로 comparator exit `3`이고 latest `bbe8a8d` artifact도 없으므로 Download 금지다.
> Reviewed source-bound artifact transition, PLC download와 실제 Connect-Close-Connect
> before/after latch+tuple+queue 및 재등록/수신 증거가 남아 있다.
