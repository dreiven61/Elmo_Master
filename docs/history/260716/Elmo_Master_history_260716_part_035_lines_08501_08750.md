...
> ```json
> {
>   "title": "Activate Motion network editor",
>   "code": "\nawait sky.click({window:targetWindow, x:88, y:142, screenshotId:\"screenshot-1\"});\nawait new Promise(r=>setTimeout(r,700));\nvar wsMotion2 = await sky.get_window_state({window:targetWindow, include_text:true});\nnodeRepl.write(JSON.stringify({focused:wsMotion2.accessibility?.focused_element, shots:wsMotion2.screenshots?.map(s=>({id:s.id,originX:s.originX,originY:s.originY,width:s.width,height:s.height}))},null,2));\n"
> }
> ```
>
> ```text
> {
>   "focused": "18 창 ID: 59648",
>   "shots": [
>     {
>       "id": "screenshot-0",
>       "originX": -1880,
...
> ```json
> {
>   "title": "View LASAL editor tabs",
>   "code": "\nawait nodeRepl.emitImage(wsMotion2.screenshots[2].url);\n"
> }
> ```
>
> Image output: image/jpeg
...
> ```json
> {
>   "title": "View LASAL workspace",
>   "code": "\nawait nodeRepl.emitImage(wsMotion2.screenshots[0].url);\n"
> }
> ```
>
> Image output: image/jpeg
...
> ```json
> {
>   "title": "Inspect LASAL automation actions",
>   "code": "\nnodeRepl.write(JSON.stringify(Object.keys(sky).sort(),null,2));\n"
> }
> ```
>
> ```text
...
> ```json
> {
>   "title": "Inspect drag action signature",
>   "code": "\nnodeRepl.write(String(sky.drag));\n"
> }
> ```
>
> ```text
...
> ```json
> {
>   "title": "Check Windows automation API",
>   "code": "\nvar skyApiDocs = await sky.documentation(\"api\");\nnodeRepl.write(String(skyApiDocs).slice(0,50000));\n"
> }
> ```
>
> ````text
