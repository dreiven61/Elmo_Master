...
> ```json
> {
>   "title": "Refresh LASAL windows",
>   "code": "globalThis.apps=await sky.list_apps(); globalThis.targetApp=apps.find(a=>a.id===targetApp.id); nodeRepl.write(JSON.stringify(targetApp.windows,null,2));"
> }
> ```
>
> ```text
...
>   "code": "globalThis.windows=await sky.list_windows(); nodeRepl.write(JSON.stringify(windows.filter(w=>/Lasal|Open a Lasal|Projectfile/i.test(`${w.title??''} ${w.app??''}`)),null,2));"
> }
> ```
>
> ```text
...
> ```json
> {
>   "title": "Target LASAL project dialog",
>   "code": "nodeRepl.write(JSON.stringify(state.screenshots.map(({id,width,height,originX,originY,zIndex})=>({id,width,height,originX,originY,zIndex})),null,2));"
> }
> ```
>
> ```text
...
> ```json
> {
>   "title": "Inspect LASAL project dialog",
>   "code": "globalThis.state=await sky.get_window_state({window:targetWindow,include_screenshot:true,include_text:false}); globalThis.targetWindow=state.window; nodeRepl.write(JSON.stringify(state.screenshots.map(({id,width,height,originX,originY,zIndex})=>({id,width,height,originX,originY,zIndex})),null,2));"
> }
> ```
>
> ```text
