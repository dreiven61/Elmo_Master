...
> ```json
> {
>   "code": "globalThis.launchResult = await sky.launch_app({app: String.raw`C:\\work\\Elmo\\Elmo_Master\\LMC_Library\\Codex_LASAL_WPF\\bin\\Debug\\LasalMotionControlApiExample.exe`}); nodeRepl.write(JSON.stringify(globalThis.launchResult, null, 2));",
>   "title": "예제 화면 실행"
> }
> ```
>
> ```text
> Windows failed to launch app (ShellExecuteW returned 2)
> ```
>
> MCP tool call
>
...
> ```json
> {
>   "code": "globalThis.launchResult = await sky.launch_app({app: String.raw`C:\\work\\Elmo\\Elmo_Master\\LMC_Library\\Codex_LASAL_WPF\\PmasApiWpfTestApp\\bin\\Debug\\LasalMotionControlApiExample.exe`}); nodeRepl.write(JSON.stringify(globalThis.launchResult, null, 2));",
>   "title": "예제 화면 실행"
> }
> ```
>
...
> ```json
> {
>   "code": "globalThis.apps = await sky.list_apps(); nodeRepl.write(JSON.stringify(globalThis.apps.filter(a => String(a.displayName || a.id).toLowerCase().includes('lasal') || (a.windows || []).some(w => String(w.title).includes('Motion Control API'))), null, 2));",
>   "title": "예제 창 찾기"
> }
> ```
>
