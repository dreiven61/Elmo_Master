...
> ```json
> {
>   "code": "globalThis.targetWindow = await sky.get_window({app: String.raw`process:C:\\work\\Elmo\\Elmo_Master\\LMC_Library\\Codex_LASAL_WPF\\PmasApiWpfTestApp\\bin\\Debug\\LasalMotionControlApiExample.exe`, id: 202024}); await sky.activate_window({window: globalThis.targetWindow}); globalThis.windowState = await sky.get_window_state({window: globalThis.targetWindow, include_screenshot: true, include_text: true}); nodeRepl.write((globalThis.windowState.accessibility?.tree || '') + '\\nScreenshots=' + globalThis.windowState.screenshots.length);",
>   "title": "초기 화면 배치 확인"
> }
> ```
>
