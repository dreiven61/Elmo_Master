...
>   "code": "await sky.click({window:cuaLasalWindow27,x:922,y:499,button:\"right\"});"
> }
> ```
>
> ```text
...
>   "code": "var cuaState300 = await sky.get_window_state({window:cuaLasalWindow27}); await nodeRepl.emitImage(cuaState300.screenshots[0].url);"
> }
> ```
>
...
> ```json
> {
>   "title": "Refresh selected channel",
>   "code": "var cuaState301 = await sky.get_window_state({window:cuaLasalWindow27}); nodeRepl.write(cuaState301.window?.title||\"\");"
> }
> ```
>
> ```text
...
> ```json
> {
>   "title": "Open LASAL context menu",
>   "code": "await sky.press_key({window:cuaLasalWindow27,key:\"F10\",modifiers:[\"SHIFT\"]});"
> }
> ```
>
> ```text
...
>   "code": "var cuaState302 = await sky.get_window_state({window:cuaLasalWindow27}); await nodeRepl.emitImage(cuaState302.screenshots[0].url);"
> }
> ```
>
> Image output: image/jpeg
...
> ```json
> {
>   "title": "Inspect context action API",
...
> ```
>
> - Ran `rg -n "perform_secondary_action|secondary" "C:\Users\dreiv\.codex\plugins\cache\openai-bundled\computer-use\26.721.30844\skills\computer-use\SKILL.md"`
>
> - Ran `Get-Content -LiteralPath 'C:\Users\dreiv\.codex\plugins\cache\openai-bundled\computer-use\26.721.30844\skills\computer-use\SKILL.md' -Raw`
>
> </details>
>
> MCP tool call
...
> ```json
> {
>   "title": "Read desktop control API",
>   "code": "var cuaApiDocs303 = await sky.documentation(\"api\"); nodeRepl.write(cuaApiDocs303);"
> }
> ```
>
> ````text
