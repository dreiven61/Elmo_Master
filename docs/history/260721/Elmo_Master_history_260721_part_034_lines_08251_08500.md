> 					300 콘솔 트리 (selectable) Elmo_1
> 					301 콘솔 트리 (selectable) Elmo_2
> 					302 콘솔 트리 (selectable) Elmo_3
> 					303 콘솔 트리 (selectable) Elmo_4
> 					304 콘솔 트리 (selectable) LMCDiagnosticsService
> 					305 콘솔 트리 (selectable) LMCEcatInputLatch
> 					306 콘솔 트리 (selectable) TCPMotionInterface
> 			307 탭 항목 (selectable) Lib
> 			308 탭 항목 (selectable) File
> 			309 탭 항목 (selectable) Global
> 			310 탭 항목 (selectable) Net
> 			311 탭 항목 (selectable) Class
> 			312 단추 Close
> 		313 Tab Properties ID: 125485008
> 			314 창 ID: 290002192
> 				315 TABLE Properties Window ID: 293314152
> 					316 스크롤 막대 세로 ID: NonClientVerticalScrollBar
> 						317 단추 위쪽 스크롤 화살표 ID: UpButton
> 						318 위치 조정 위치 ID: ScrollbarThumb
> 						319 단추 페이지 아래로 ID: DownPageButton
> 						320 단추 아래쪽 스크롤 화살표 ID: DownButton
> 					321 custom Name
> 					322 custom Revision
> 					323 custom GUID
> 					324 custom Task Settings
> 					325 custom Sigmatek
> 					326 custom OSInterface
> 					327 custom IconPath
> 					328 custom SharedCommandTable
> 					329 custom Objectsize
> 					330 custom Singleton
> 					331 custom Hidden
> 					332 custom Deprecated
> 					333 custom GCCOptions
> 					334 custom Comment
> 					335 custom Filename
> 				336 도구 모음 ID: 59392
> 					337 단추
> 					338 단추
> 			339 탭 항목 (selectable) Properties
> 			340 단추 Close
>
> The focused UI element is 295 트리 ID: 103.
> ```
>
...
> ```json
> {
>   "title": "LASAL EtherCAT master 채널 이름 입력",
>   "code": "{const observation=globalThis.state;if(!observation)throw new Error(\"No fresh observation\");globalThis.state=null;try{await sky.type_text({window:observation.window,text:\"EcatMaster\"});await sky.press_key({window:observation.window,key:\"ENTER\"});globalThis.state=await sky.get_window_state({window:observation.window,include_screenshot:true,include_text:true});}catch(e){globalThis.state=await sky.get_window_state({window:observation.window,include_screenshot:true,include_text:true});throw e;}globalThis.targetWindow=globalThis.state.window;nodeRepl.write(String(globalThis.state.accessibility?.tree||globalThis.state.accessibility?.document_text||\"\"));}"
> }
> ```
>
> ```text
