...
> ```json
> {
>   "code": "var apiDoc2=await sky.documentation(\"api\"); nodeRepl.write(String(apiDoc2).slice(0,30000));",
...
> ```json
> {
>   "code": "await sky.click({window:globalThis.targetWindow,x:651,y:278,mouse_button:\"right\",screenshotId:\"screenshot-0\"});\nawait new Promise(r=>setTimeout(r,400));\nglobalThis.state=await sky.get_window_state({window:globalThis.targetWindow,include_screenshot:true,include_text:true});\nnodeRepl.write(globalThis.state.accessibility?.tree||\"\");\nif(globalThis.state.screenshots?.[0]?.url) await nodeRepl.emitImage(globalThis.state.screenshots[0].url);",
...
> 		1 창 작업 영역 ID: 59648
> 			2 창 Comm_Network Secondary Actions: Raise ID: 65283
> 				3 창 ID: 59648
...
> 					15 위치 조정 (disabled)
> 			16 창 Motion_Network Secondary Actions: Raise ID: 65284
> 				17 창 ID: 59648
...
> 					28 위치 조정 (disabled)
> 			29 창 LMCEcatInputLatch Secondary Actions: Raise ID: 65287
> 				30 창 ID: 59648
