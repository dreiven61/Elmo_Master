> The focused UI element is 264 TABLE Properties Window ID: 297584112.
> ```
>
...
> ```json
> {
>   "code": "var o16=globalThis.state;globalThis.state=null;await sky.click({window:o16.window,element_index:248,mouse_button:'right'});globalThis.state=await sky.get_window_state({window:o16.window,include_screenshot:true,include_text:true});globalThis.targetWindow=state.window;nodeRepl.write(String(state.accessibility?.tree||state.accessibility?.document_text||''));",
>   "title": "다음 RT latch 변수 추가"
> }
> ```
>
> ```text
