> 		282 창 Splitter ID: 125724144
> 		283 Tab Class View ID: 125483184
> 			284 트리 ID: 103
> 				285 콘솔 트리 (selectable) Elmo_EtherCAT_Test_4Axis
> 					286 콘솔 트리 (selectable) External
> 					287 콘솔 트리 (selectable) Sigmatek
> 					288 콘솔 트리 (selectable) _TCPIPServer_RT
> 					289 콘솔 트리 (selectable) Elmo_1
> 					290 콘솔 트리 (selectable) Elmo_2
> 					291 콘솔 트리 (selectable) Elmo_3
> 					292 콘솔 트리 (selectable) Elmo_4
> 					293 콘솔 트리 (selectable) LMCEcatInputLatch
...
> 					309 custom Project Version
> 					310 custom Name
> 					311 custom Revision
> 					312 custom Online Connection
> 					313 custom CodeGenerator
> 					314 custom Networks
> 					315 custom Server list
> 					316 custom Target
> 					317 custom Compiler
> 					318 custom Directory
> 					319 custom OPC-UA
> 					320 custom Load all libraries
> 					321 custom Use Unit System
> 					322 custom Include Paths
> 					323 custom Library Paths
> 					324 custom Backup Includes and Loader
> 					325 custom Ignore at Cleanup
> 					326 custom Reencrypt Project On Close
> 					327 custom Enable OPC UA
> 					328 custom Enable initvalues for output parameters
> 					329 custom Use multiple CPU core
> 					330 custom Use Advanced-IO
> 					331 custom AutomationML
> 					332 custom IO Connection Manager Options
> 					333 custom Comment
> 					334 custom Filename
> 				335 도구 모음 ID: 59392
> 					336 단추
> 					337 단추
> 			338 탭 항목 (selectable) Properties
> 			339 단추 Close
>
> The focused UI element is 284 트리 ID: 103.
> ```
>
...
> ```json
> {
>   "title": "LASAL 진단 클래스 이름 입력",
>   "code": "{const observation=globalThis.state;if(!observation)throw new Error(\"No fresh observation\");globalThis.state=null;try{await sky.type_text({window:observation.window,text:\"LMCDiagnosticsService\"});globalThis.state=await sky.get_window_state({window:observation.window,include_screenshot:false,include_text:true});}catch(e){globalThis.state=await sky.get_window_state({window:observation.window,include_screenshot:true,include_text:true});throw e;}globalThis.targetWindow=globalThis.state.window;nodeRepl.write(String(globalThis.state.accessibility?.tree||globalThis.state.accessibility?.document_text||\"\"));}"
> }
> ```
>
> ```text
...
> ```json
> {
>   "title": "LASAL 진단 클래스 확정",
>   "code": "{const observation=globalThis.state;if(!observation)throw new Error(\"No fresh observation\");globalThis.state=null;try{await sky.press_key({window:observation.window,key:\"ENTER\"});globalThis.state=await sky.get_window_state({window:observation.window,include_screenshot:true,include_text:true});}catch(e){globalThis.state=await sky.get_window_state({window:observation.window,include_screenshot:true,include_text:true});throw e;}globalThis.targetWindow=globalThis.state.window;nodeRepl.write(String(globalThis.state.accessibility?.tree||globalThis.state.accessibility?.document_text||\"\"));}"
> }
> ```
>
> ```text
