...
> Status: completed
>
> PDO 매핑은 LASAL 소스에서 확인할 수 있고, 지금은 그 매핑을 그대로 진단 Catalog의 기준값으로 고정하는 중입니다. 동시에 PC API가 이 Catalog의 CRC·축 순서·응답 크기를 실제 계약대로 검증하도록 보강하고 있습니다.
>
...
>   "code": "{const observation=globalThis.state;if(!observation)throw new Error(\"No fresh observation\");globalThis.state=null;try{await sky.press_key({window:observation.window,key:\"Shift_L+F10\"});globalThis.state=await sky.get_window_state({window:observation.window,include_screenshot:true,include_text:true});}catch(e){globalThis.state=await sky.get_window_state({window:observation.window,include_screenshot:true,include_text:true});throw e;}globalThis.targetWindow=globalThis.state.window;nodeRepl.write(String(globalThis.state.accessibility?.tree||globalThis.state.accessibility?.document_text||\"\"));}"
> }
> ```
>
> ```text
