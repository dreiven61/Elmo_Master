...
> ```json
> {
>   "code": "var o6=globalThis.state;globalThis.state=null;await sky.click({window:o6.window,element_index:248});globalThis.state=await sky.get_window_state({window:o6.window,include_screenshot:true,include_text:true});globalThis.targetWindow=state.window;nodeRepl.write(String(state.accessibility?.tree||state.accessibility?.document_text||''));",
>   "title": "RT latch 클래스 선택"
> }
> ```
>
> ```text
