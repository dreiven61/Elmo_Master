>
> 최종 단계는 LASAL IDE를 실제로 조작해야 하므로 `computer-use` 절차를 적용하겠습니다. 외부에서 복구한 `.st`가 IDE 내부 클래스와 다시 동기화되는지, Rebuild 후에도 해시가 유지되는지를 화면·로그·파일로 함께 확인하는 데 사용합니다.
>
...
>   "code": "if (!globalThis.sky) { var computerUseModule = await import('C:/Users/dreiv/.codex/plugins/cache/openai-bundled/computer-use/26.715.52143/scripts/computer-use-client.mjs'); await computerUseModule.setupComputerUseRuntime({ globals: globalThis }); }\nvar guidanceText = await sky.documentation('guidance');\nvar confirmationsText = await sky.documentation('confirmations');\nnodeRepl.write(guidanceText);\nnodeRepl.write(confirmationsText);"
> }
> ```
>
> ````text
