...
> ```json
> {
>   "code": "globalThis.apps = await sky.list_apps();\nglobalThis.targetApp = apps.find(a=>a.id==='process:C:\\\\Program Files (x86)\\\\Sigmatek\\\\Lasal\\\\Class2\\\\Bin\\\\Lasal2.exe');\nif(!targetApp) throw new Error('LASAL app not returned');\nvar candidates = targetApp.windows.filter(w=>String(w.title||'').includes('Elmo_EtherCAT_Test_4Axis'));\nnodeRepl.write(JSON.stringify(candidates.map(w=>({id:w.id,app:w.app,title:w.title})),null,2));",
>   "title": "LASAL 프로젝트 창 확인"
> }
> ```
>
> ```text
> LASAL app not returned
> ```
>
> MCP tool call
>
...
> ```json
> {
>   "code": "globalThis.windows = await sky.list_windows(); nodeRepl.write(JSON.stringify(windows.map(w=>({id:w.id,app:w.app,title:w.title})).filter(x=>String(x.title).includes('Lasal')||String(x.app).includes('Lasal')),null,2));",
>   "title": "열린 LASAL 창 탐색"
> }
> ```
>
...
> ```json
> {
>   "code": "await sky.launch_app({app:'C:\\\\Program Files (x86)\\\\Sigmatek\\\\Lasal\\\\Class2\\\\Bin\\\\Lasal2.exe'}); nodeRepl.write('launched');",
...
> ```json
> {
>   "code": "await new Promise(r=>setTimeout(r,2000)); globalThis.apps=await sky.list_apps(); nodeRepl.write(JSON.stringify(apps.filter(a=>String(a.id).includes('Lasal2')).map(a=>({id:a.id,windows:a.windows.map(w=>({id:w.id,title:w.title,app:w.app}))})),null,2));",
>   "title": "LASAL 시작 확인"
> }
> ```
>
