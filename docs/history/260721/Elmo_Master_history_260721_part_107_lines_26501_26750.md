...
> 							212 단추 페이지 위로 ID: UpPageButton
> 							213 위치 조정 위치 ID: ScrollbarThumb
...
> 					260 단추
> 					261 단추
> 			262 탭 항목 (selectable) Properties
> 			263 단추 Close
>
...
> ```json
> {
>   "code": "var o4=globalThis.state; var sid4=o4.screenshots?.[0]?.id;if(!sid4)throw new Error('no screenshot');globalThis.state=null;await sky.click({window:o4.window,screenshotId:sid4,x:700,y:870});globalThis.state=await sky.get_window_state({window:o4.window,include_screenshot:true,include_text:true});globalThis.targetWindow=state.window;nodeRepl.write(String(state.accessibility?.focused_element||''));",
>   "title": "LASAL 컴파일 출력 선택"
> }
> ```
>
> ```text
...
> ```json
> {
>   "code": "var o5=globalThis.state;globalThis.state=null;await sky.press_key({window:o5.window,key:'Control_L+Home'});globalThis.state=await sky.get_window_state({window:o5.window,include_screenshot:true,include_text:true});globalThis.targetWindow=state.window;nodeRepl.write(String(state.accessibility?.tree||state.accessibility?.document_text||''));",
>   "title": "LASAL 오류 위치 찾기"
> }
> ```
>
> ```text
