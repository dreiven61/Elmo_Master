...
> 			280 단추 Close
> 		281 Tab Properties ID: 409871640
> 			282 창 ID: 121918456
> 				283 TABLE Properties Window ID: 127184512
> 					284 스크롤 막대 세로 ID: NonClientVerticalScrollBar
> 						285 단추 위쪽 스크롤 화살표 ID: UpButton
> 						286 위치 조정 위치 ID: ScrollbarThumb
> 						287 단추 페이지 아래로 ID: DownPageButton
> 						288 단추 아래쪽 스크롤 화살표 ID: DownButton
> 					289 custom Project Version
> 					290 custom Name
> 					291 custom Revision
> 					292 custom Online Connection
> 					293 custom CodeGenerator
> 					294 custom Networks
> 					295 custom Server list
> 					296 custom Target
> 					297 custom Compiler
> 					298 custom Directory
> 					299 custom OPC-UA
> 					300 custom Load all libraries
> 					301 custom Use Unit System
> 					302 custom Include Paths
> 					303 custom Library Paths
> 					304 custom Backup Includes and Loader
> 					305 custom Ignore at Cleanup
> 					306 custom Reencrypt Project On Close
> 					307 custom Enable OPC UA
> 					308 custom Enable initvalues for output parameters
> 					309 custom Use multiple CPU core
> 					310 custom Use Advanced-IO
> 					311 custom AutomationML
> 					312 custom IO Connection Manager Options
> 					313 custom Comment
> 					314 custom Filename
> 				315 도구 모음 ID: 59392
> 					316 단추
...
> 			319 단추 Close
>
> The focused UI element is 273 트리 ID: 105.
> ```
>
...
> ```json
> {
>   "code": "globalThis.state=await sky.get_window_state({window:globalThis.targetWindow,include_screenshot:false,include_text:true}); var typeLine=(globalThis.state.accessibility?.tree||\"\").split(\"\\n\").find(x=>/콘솔 트리 \\(selectable\\) Types$/.test(x)); nodeRepl.write(typeLine||\"Types not found\");",
>   "title": "Locate LASAL type tree",
>   "timeout_ms": 30000
> }
> ```
>
...
> ```json
> {
>   "code": "await sky.click({window:globalThis.targetWindow,element_index:275,click_count:2}); nodeRepl.write(\"expanded Types\");",
>   "title": "Expand LASAL types",
>   "timeout_ms": 30000
> }
> ```
>
...
> ```json
> {
>   "code": "globalThis.state=await sky.get_window_state({window:globalThis.targetWindow,include_screenshot:true,include_text:true}); var typeTree=(globalThis.state.accessibility?.tree||\"\"); nodeRepl.write(typeTree.split(\"\\n\").filter(x=>/LMCRecorder|TCPMotion|LMCDiagnostics|LMCEcat|Types|Class/.test(x)).slice(-200).join(\"\\n\"));",
>   "title": "Inspect LASAL classes",
>   "timeout_ms": 30000
> }
> ```
>
...
> ```json
> {
>   "code": "var shotTypes=globalThis.state.screenshots.sort((a,b)=>b.zIndex-a.zIndex)[0]; await sky.click({window:globalThis.targetWindow,x:963,y:184,screenshotId:shotTypes.id}); nodeRepl.write(\"clicked Types expand control\");",
>   "title": "Expand LASAL type list",
>   "timeout_ms": 30000
> }
> ```
>
> ```text
> clicked Types expand control
> ```
>
> MCP tool call
>
...
> ```json
> {
>   "code": "globalThis.state=await sky.get_window_state({window:globalThis.targetWindow,include_screenshot:true,include_text:true}); var classLines=(globalThis.state.accessibility?.tree||\"\").split(\"\\n\").filter(x=>/LMCRecorder|TCPMotion|LMCDiagnostics|LMCEcat|Types|CLASS|Class/.test(x)); nodeRepl.write(classLines.slice(-240).join(\"\\n\"));",
>   "title": "Read LASAL type list",
>   "timeout_ms": 30000
> }
> ```
>
...
> ```json
> {
>   "code": "var shotLmc=globalThis.state.screenshots.sort((a,b)=>b.zIndex-a.zIndex)[0]; await sky.click({window:globalThis.targetWindow,x:977,y:225,screenshotId:shotLmc.id}); nodeRepl.write(\"expanded _LMC types\");",
>   "title": "Expand LASAL LMC classes",
>   "timeout_ms": 30000
> }
> ```
>
...
> ```json
> {
>   "code": "globalThis.state=await sky.get_window_state({window:globalThis.targetWindow,include_screenshot:true,include_text:true}); var targets=(globalThis.state.accessibility?.tree||\"\").split(\"\\n\").filter(x=>/LMCRecorder|TCPMotion|LMCDiagnostics|LMCEcat|_LMC/.test(x)); nodeRepl.write(targets.slice(-240).join(\"\\n\"));",
>   "title": "Locate changed LASAL classes",
>   "timeout_ms": 30000
> }
> ```
>
