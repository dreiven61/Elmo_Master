> 						255 단추 왼쪽 스크롤 화살표 ID: UpButton
> 						256 위치 조정 위치 ID: ScrollbarThumb
> 						257 단추 오른쪽 스크롤 화살표 ID: DownButton
> 			258 탭 항목 (selectable) Python Script
> 			259 탭 항목 (selectable) Output
> 			260 탭 항목 (selectable) Debugger
> 			261 단추 Close
> 		262 창 Splitter ID: 302436696
> 		263 Tab Class View ID: 297479384
> 			264 트리 ID: 103
> 				265 콘솔 트리 (selectable) Elmo_EtherCAT_Test_4Axis
> 					266 콘솔 트리 (selectable) External
> 					267 콘솔 트리 (selectable) Sigmatek
> 					268 콘솔 트리 (selectable) _TCPIPServer_RT
> 					269 콘솔 트리 (selectable) Elmo_1
> 					270 콘솔 트리 (selectable) Elmo_2
> 					271 콘솔 트리 (selectable) Elmo_3
> 					272 콘솔 트리 (selectable) Elmo_4
> 					273 콘솔 트리 (selectable) LMCDiagnosticsService
> 					274 콘솔 트리 (selectable) LMCEcatInputLatch
> 						275 콘솔 트리 (selectable) Servers
> 						276 콘솔 트리 (selectable) Clients
> 						277 콘솔 트리 (selectable) Methods
> 						278 콘솔 트리 (selectable) Dependencies
...
> 			285 단추 Close
> 		286 Tab Properties ID: 302395640
> 			287 창 ID: 291858232
> 				288 TABLE Properties Window ID: 297584112
...
> 					295 custom Revision
> 					296 custom GUID
> 					297 custom Task Settings
> 					298 custom Sigmatek
> 					299 custom OSInterface
> 					300 custom IconPath
> 					301 custom SharedCommandTable
> 					302 custom Objectsize
> 					303 custom Singleton
> 					304 custom Hidden
> 					305 custom Deprecated
> 					306 custom GCCOptions
> 					307 custom Comment
> 					308 custom Filename
> 				309 도구 모음 ID: 59392
> 					310 단추
> 					311 단추
> 			312 탭 항목 (selectable) Properties
> 			313 단추 Close
>
> The focused UI element is 264 트리 ID: 103.
> ```
>
...
> ```json
> {
>   "code": "var o10=globalThis.state;globalThis.state=null;await sky.click({window:o10.window,element_index:19});globalThis.state=await sky.get_window_state({window:o10.window,include_screenshot:true,include_text:true});globalThis.targetWindow=state.window;nodeRepl.write(String(state.accessibility?.tree||state.accessibility?.document_text||''));",
>   "title": "RT latch 변수 추가"
> }
> ```
>
> Image output: image/jpeg
...
> ```json
> {
>   "code": "var o11=globalThis.state;globalThis.state=null;await sky.type_text({window:o11.window,text:'PublishSequence'});globalThis.state=await sky.get_window_state({window:o11.window,include_screenshot:true,include_text:true});globalThis.targetWindow=state.window;nodeRepl.write(String(state.accessibility?.focused_element||''));",
>   "title": "RT latch 변수 이름 입력"
> }
> ```
>
> ```text
...
> ```json
> {
>   "code": "var o12=globalThis.state;globalThis.state=null;await sky.press_key({window:o12.window,key:'ENTER'});globalThis.state=await sky.get_window_state({window:o12.window,include_screenshot:true,include_text:true});globalThis.targetWindow=state.window;nodeRepl.write(String(state.accessibility?.tree||state.accessibility?.document_text||''));",
>   "title": "RT latch 변수 생성 확인"
> }
> ```
>
> ```text
