...
> 					272 콘솔 트리 (selectable) External
> 					273 콘솔 트리 (selectable) Sigmatek
> 					274 콘솔 트리 (selectable) _TCPIPServer_RT
> 					275 콘솔 트리 (selectable) Elmo_1
> 					276 콘솔 트리 (selectable) Elmo_2
> 					277 콘솔 트리 (selectable) Elmo_3
> 					278 콘솔 트리 (selectable) Elmo_4
> 					279 콘솔 트리 (selectable) LMCDiagnosticsService
> 					280 콘솔 트리 (selectable) LMCEcatInputLatch
> 						281 콘솔 트리 (selectable) Servers
> 						282 콘솔 트리 (selectable) Clients
> 							283 콘솔 트리 (selectable) Client
> 						284 콘솔 트리 (selectable) Methods
> 					285 콘솔 트리 (selectable) TCPMotionInterface
> 			286 탭 항목 (selectable) Lib
> 			287 탭 항목 (selectable) File
> 			288 탭 항목 (selectable) Global
> 			289 탭 항목 (selectable) Net
> 			290 탭 항목 (selectable) Class
> 			291 단추 Close
> 		292 Tab Properties ID: 125485008
> 			293 창 ID: 290002192
...
> 			319 단추 Close
>
> The focused UI element is 270 편집 ID: 1.
> ```
>
...
> ```json
> {
>   "title": "LASAL 입력 래치 채널 구성",
>   "code": "{const observation=globalThis.state;if(!observation)throw new Error(\"No fresh observation\");globalThis.state=null;try{await sky.click({window:observation.window,x:1021,y:349});globalThis.state=await sky.get_window_state({window:observation.window,include_screenshot:false,include_text:true});}catch(e){globalThis.state=await sky.get_window_state({window:observation.window,include_screenshot:true,include_text:true});throw e;}globalThis.targetWindow=globalThis.state.window;nodeRepl.write(String(globalThis.state.accessibility?.tree||globalThis.state.accessibility?.document_text||\"\"));}"
> }
> ```
>
> ```text
