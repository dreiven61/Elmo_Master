...
> 					294 스크롤 막대 세로 ID: NonClientVerticalScrollBar
> 						295 단추 위쪽 스크롤 화살표 ID: UpButton
> 						296 위치 조정 위치 ID: ScrollbarThumb
> 						297 단추 페이지 아래로 ID: DownPageButton
> 						298 단추 아래쪽 스크롤 화살표 ID: DownButton
> 					299 custom Name
> 					300 custom Revision
> 					301 custom GUID
> 					302 custom Task Settings
> 					303 custom Sigmatek
> 					304 custom OSInterface
> 					305 custom IconPath
> 					306 custom SharedCommandTable
> 					307 custom Objectsize
> 					308 custom Singleton
> 					309 custom Hidden
> 					310 custom Deprecated
> 					311 custom GCCOptions
> 					312 custom Comment
> 					313 custom Filename
> 				314 도구 모음 ID: 59392
> 					315 단추
> 					316 단추
> 			317 탭 항목 (selectable) Properties
> 			318 단추 Close
>
> The focused UI element is 293 TABLE Properties Window ID: 293314152.
> ```
>
> MCP tool call
>
...
>   "code": "{const observation=globalThis.state;if(!observation)throw new Error(\"No fresh observation\");globalThis.state=null;try{await sky.click({window:observation.window,x:1008,y:404});globalThis.state=await sky.get_window_state({window:observation.window,include_screenshot:true,include_text:true});}catch(e){globalThis.state=await sky.get_window_state({window:observation.window,include_screenshot:true,include_text:true});throw e;}globalThis.targetWindow=globalThis.state.window;nodeRepl.write(String(globalThis.state.accessibility?.tree||globalThis.state.accessibility?.document_text||\"\"));}"
> }
> ```
>
> ```text
