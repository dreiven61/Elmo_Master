...
- 현재 진행도: 50% + `MODE-08` 진행 중
- current 상태: `Partial/Limited`; PC/SDK contract와 PLC runtime의 MODE-06/07까지 구현, MODE-08 safety preemption source 반영
- 구현된 SDK command: `0x7D23 Start`, `0x7D24 ReadOutcome`, `0x7D25 Retire`
- 구현된 PLC route/runtime: Diagnostics route, owner kind/resource, `6061 -> 6060 -> 6061`, durable outcome, write-dispatch 이후 read-only recovery
...
- 문서 checkpoint: 2026-08-24, `dev`의 MODE-08 safety-preemption source까지 반영
...
구현돼 있다. 그러나 public activation은 아직 하지 않는다. capability bits 8/9/10과
...
현재 PLC implementation은 MODE-06/07 기준으로 위 기본 상태 머신을 구현한다.
...
현재 source checkpoint의 원칙은 다음과 같다.
...
- `0x6060` irreversible-dispatch evidence가 있으면 safety cleanup 중 새 `0x6060` write를 절대 시작하지 않는다.
...
- 이 단계의 구현은 MODE-08 전체 완료 판정이 아니라 safety-preemption portion의 source checkpoint다.
...
MODE-08에서는 safety Stop/Power가 kind6 owner를 preempt하고 정확한 cleanup/quarantine receipt를
남기는 경로를 추가한다. generic D5와의 양방향 admission/conflict matrix는 MODE-08 완료 전에
최종 정적 검토와 C78 검증을 남긴다.

축의 current mode가 8이 아니거나 mode outcome이 unresolved이면 ordinary LMC motion을
승인하지 않는 interlock은 activation 전에 끝까지 검증한다. PLC startup에서는 6061을
...
generic D5 Write의 permanent unsafe object 목록에 `0x6060`을 추가하는 것은 별도 MODE-09다.
...
이미 dispatch된 durable Running record의 read-only recovery/cleanup은 허용하되 신규 mutation은
시작하지 않는 것이 activation gate의 의미다.
...
capability bits 8/9/10도 OFF였다. 아래 private ABI는 이후 MODE-06/07 implementation에서도
그대로 유지한다.
...
- warm-start는 write-dispatched durable Running record만 reconstruct한다.
...
- [x] `MODE-02` command ID/capability bit와 PLC owner kind 6/resource 4 ABI freeze
...
- [ ] `MODE-08` Home/SetPosition/motion/generic SDO ownership conflict 구현 — 진행 중; kind6 safety preemption source 반영
...
### 8.3 MODE-06/07 C78/PLC checkpoint
...
- 이 수정이 포함된 MODE-06 source는 C78 build, PLC download와 기본 정상동작을 통과했다.
- MODE-07 source는 `WriteDispatched` 이후 `WRITE_START`로 복귀하지 않는 guard와
  `RECOVERY_START/RECOVERY_WAIT` read-only path를 추가했다.
- 최신 MODE-08 변경은 별도 C78 build/download 검증이 아직 필요하므로 MODE-10 완료 근거로 사용하지 않는다.

### 8.4 2026-08-24 MODE-08 source checkpoint

- current `dev`에는 SetOperationMode safety preemption handling source가 반영돼 있다.
- Control의 preempted-owner surface가 kind6/command `0x7D23`/resource4/identity56/active-state12를
  인식하도록 확장됐다.
- Diagnostics `ProcessAxisSetOperationMode`는 preemption snapshot을 읽고 exact old/safety tuple을
  검증하며 pending-freeze/cleanup/quarantine을 처리한다.
- write-dispatched state에서는 safety preemption 처리 중 새 `0x6060` mutation을 시작하지 않는다.
- 이 checkpoint는 source-level MODE-08 진행 증거다. 전체 conflict matrix, generic D5 admission,
  C78 compile/download 및 hardware packet 검증을 끝내기 전에는 MODE-08을 `[x]`로 바꾸지 않는다.
