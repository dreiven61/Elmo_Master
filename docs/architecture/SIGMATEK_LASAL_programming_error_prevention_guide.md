...
합쳐 client count `11`이다. 일반 `_TCPIPServer1` 연결을 사용하고
`TCPMotionInterface1`은 CyclicTime 1 ms만 사용하며 RealTime assignment를 두지
않는다. 이것은 source/network 상태이며 9축 변경 뒤 IDE Rebuild 증거는 없다.
