...
현재 tracked source에는 `LMCAxis1..9`, object registry, depth-8 request queue,
...
현재 tracked class table은 motion client `LMCAxis1..9`와 `LMCRobot`, `_StdLib`을
합쳐 client count `11`이다. 편집 가능한 `TCPIPServer1 : TCPIPServer` 연결을
사용하고 `TCPMotionInterface1`은 CyclicTime 1 ms만 사용하며 RealTime assignment를
두지 않는다. `TCPIPServer`도 custom RT task나 CyWork override 없이 inherited
`_TCPIPServer::CyWork`를 사용한다. 이것은 source/network 상태이며 최신 변경 뒤
IDE Rebuild 증거는 없다.
다음 기능 구현 전에 아래 잔여 gate를 완료해야 한다.

1. 최종 저장된 `Motion_Network.lcn`에서
   `TCPMotionInterface1.LMCAxis1 -> _LMCAxis1.Control`과 축 2..9 연결을
   strict contract로 확인한다.
2. interface/server를 같은 CyWork task에 두고 `Config=0`,
   `MaxConnections=2`, `ConnectionsPerRun=1`을 적용한다. 두 번째 슬롯은
   same-peer reconnect candidate 비교 전용이고 stable RPC owner는 하나인지
   확인한다. interface CyWork는 axis RT thread와 같은 CPU core에서 같거나 낮은
   priority인지 확인한다.
...
- 실제 `_LMCAxis1..9`와 group object 이름은 LASAL이 Network 연결을 통해
  조회한다.
...
- 현재 tracked `_LMCAxis1..9`의 application transmission은
  `ExUnits=8388608`, `IntUnits=1 mm(10000)`다.
...
- `LMCAXIS_PAR_SET_MAXMODULO`는 Init 종료 전에만 설정하며, 공식 허용식
  `Value x ExUnits / IntUnits <= 2147483647`을 만족해야 한다. 현재 tracked
  비율에서는 최대 약 `2,559,999 DINT = 255.9999 mm`다.
...
- TCP/parser를 변경했다면 packet의 command ID, endian, length, offset이 PC와
  일치한다.
- Axis, DS402, PDO 또는 Network를 변경했다면 `_LMCAxis1..9`, 실제축 1..4의
  DS402/PDO, 가상축 5..9의 SimulateMode와 4축 Cartesian group, Motion
  Network의 관련 연결을 확인했다.
...
현재 상태는 `9축 source/network 및 정적 계약 반영 완료, LASAL IDE Rebuild와
PLC 동작 시험 대기`로 기록한다. 이 상태는 production 완료가 아니다.
