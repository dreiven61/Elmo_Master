...
typed RT mailbox, `DataHandling()`/`SendData()` override가 source-first 형태로
반영됐다. 다음 기능 구현 전에 LASAL IDE에서 다음을 완료해야 한다.

1. `TCPMotionInterface` class model의 첫 client를 `LMCAxis1`로 바꾸고 새
   queue/mailbox/session/fault 변수를 등록한다.
2. CodeGenerator로 재생성한다.
3. 생성된 client descriptor/hash가 tracked source와 일치하는지 확인한다.
4. class table header의 client count가 `6`인지 확인한다.
5. `Motion_Network.lcn`에서
   `TCPMotionInterface1.LMCAxis1 -> _LMCAxis1.Control`과 나머지 세 축 연결을
   확인한다.
6. interface/server CyWork task, interface RtWork task/core, `Config=0`,
   `MaxConnections=1`을 적용한다.
...
- tracked `TCPMotionInterface.st`의 `Response()`는 callback payload를 종료 전에
  ...
  호출하지 않는다.
- `CyWork()`가 session/order/non-RT command를 처리하고 typed mailbox로
  `RtWork()`에 넘긴다. 실제 RT client call은 `0x202E ReadActualPosition` 하나만
  열려 있다.
- Power, Stop, Reset, Move와 group client-call command는 mailbox 이관 전까지
  pre-case guard에서 deterministic error `-5`를 반환한다. 기존 handler body가
  남아 있다는 이유로 runtime 허용 상태로 판단하면 안 된다.
- source 구현은 완료됐지만 LASAL IDE model/network 재생성, task/core 배치,
  compile, PLC download와 packet 재캡처는 남아 있다. 상세 적용 상태는
  [`LASAL_SOURCE_QUEUE_AND_NETWORK_APPLY_PLAN_2026-07-10.md`](../../LMC_Library/LMC_API_Delivery/docs/LASAL_SOURCE_QUEUE_AND_NETWORK_APPLY_PLAN_2026-07-10.md)를
  따른다.
...
LASAL handler가 없는 명령에 dummy success를 반환하지 않는다. 특히 현재
backlog에 남은 `0x2051`, `0x20E7`, 실제 callback event sender, multi-PC
ownership은 구현 및 PLC 검증 전까지 deterministic error로 처리한다. 현재
상태는
...
PLC 또는 실제 장비 검증을 수행하지 못했다면 상태를 `source/IDE 구현 완료,
PLC 검증 대기`로 기록한다. 이 상태는 production 완료가 아니다.
