...
| 2 | `SetOpMode` | 25% | PC/SDK contract 완료; DS402 mode 소유권과 PLC lifecycle 미구현 | [SET_OPERATION_MODE_DESIGN.md](SET_OPERATION_MODE_DESIGN.md) |
| 3 | `HomeDS402Ex` | 0% | 기존 Home과 분리된 확장 Homing 신규 구현 | [HOME_DS402_EX_DESIGN.md](HOME_DS402_EX_DESIGN.md) |
| 4 | `SetPosition` | 25% | durable backend와 RT exactly-once 후속 구현 | [SET_POSITION_DESIGN.md](SET_POSITION_DESIGN.md) |
...
| SetOpMode | `0x7D23` | `0x7D24` | `0x7D25` | C# source/golden wire 반영; LASAL route 미반영 |
| SetPosition | `0x7D12` | `0x7D14` | `0x7D1A` | current source/wire에 존재 |
...
- SetOpMode: 완료된 public model/wire 위에 mode-owner state와 fail-closed LASAL scaffold를 구현한다.
...
- SetOpMode: exact SDO write/readback와 terminal store를 구현하되 capability는 OFF로 유지한다.
