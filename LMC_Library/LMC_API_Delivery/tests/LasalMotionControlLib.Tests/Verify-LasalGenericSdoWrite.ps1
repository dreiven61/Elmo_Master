...
Require-Match $sdk 'case 0x6040:\s*case 0x6060:\s*case 0x607A:\s*case 0x60FF:\s*case 0x6071:' `
...
Require-Match $sdk 'request\.ObjectIndex == 0x2F00.*?request\.SubIndex == 24' `
...
Require-Match $plc '\(ObjectIndex = 0x6040\) \| \(ObjectIndex = 0x6060\).*?\(ObjectIndex = 0x607A\).*?\(ObjectIndex = 0x60FF\).*?\(ObjectIndex = 0x6071\)' `
    'PLC semantic-owner raw-write block list is incomplete.'
Require-Match $plc 'case ValueType of\s*1, 9, 10, 11:.*?DataLength <> 1.*?2, 3, 7:.*?DataLength <> 2.*?4, 5, 6, 8:.*?DataLength <> 4' `
    'PLC canonical 1/2/4-byte scalar type/length admission is missing.'
Require-Match $plc '\(ValueType = 1\).*?\(WriteData <> 0\).*?\(WriteData <> 1\)' `
    'PLC canonical Bool Write validation is missing.'
Require-Match $plc '\(ObjectIndex = 0x2F00\) & \(SubIndex = 24\).*?writeValue := WriteData\$DINT' `
    'PLC UI[24] range guard is not scoped to the known preset.'
Require-Match $plc 'if LMC_DIAG_D5_SDO_WRITE_GLOBAL_ENABLED = TRUE then\s*\(pResponse \+ 20\)\^\$UDINT :=\s*\(pResponse \+ 20\)\^\$UDINT OR 0x00000200;' `
