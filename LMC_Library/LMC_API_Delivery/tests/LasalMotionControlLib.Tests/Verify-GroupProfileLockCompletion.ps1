...
Check ($status -match '(?is)if\s*\(powerIsOn\s*<>\s*0\)\s*&\s*\(profileLocked\s*=\s*TRUE\)\s*then') 'Standby branch is admitted from PowerOn plus LockState'
Check ($status -match '(?is)LMCAxis1\.ReadAxisStatus\(\).*?LMCAxis2\.ReadAxisStatus\(\).*?LMCAxis3\.ReadAxisStatus\(\).*?LMCAxis4\.ReadAxisStatus\(\).*?groupMotionComplete\s*:=\s*\(groupReadInPosition\s*<>\s*0\)\s*&\s*groupAxesStandstill') 'Group motion completion combines ProfileFinished with Cartesian member Standstill'
Check ($status -match '(?is)if\s+groupAxesStandstill\s+then\s*groupReadState\s*:=\s*groupReadState\s+or\s+0x00020000') 'Standby preserves lock-ready at standstill and exposes legacy motion activity transitions'
Check (-not ($status -match '(?is)OwnershipState\[26\].*?groupReadState\s*:=\s*groupReadState\s+or\s+0x00020000')) 'Standby does not depend on the per-request dispatch marker cleared by status reads'
Check (-not ($status -match '(?is)if\s*\(powerIsOn\s*<>\s*0\)\s*&\s*\(profileLocked\s*=\s*TRUE\)\s*&\s*\(groupReadInPosition\s*<>\s*0\)')) 'First Lock Ready does not depend directly on ProfileFinished'
Check ($status -match '(?is)if\s+groupMotionComplete\s+then\s*groupReadState\s*:=\s*groupReadState\s+or\s+0x00080000') 'Verified Group motion completion is published independently as the local Group InPosition bit'
