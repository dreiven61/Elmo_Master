...
namespace LasalMotionControlApiExample
{
    ...
    internal sealed class MotionUncertaintyJournal : IDisposable
    {
        ...
        internal MotionUncertaintyRecord ResolveOperatorRetirement(
            ...
            DateTime updatedUtc)
        {
            ...
            {
                throw new InvalidOperationException(
                    "Motion retirement requires a durably committed ledger decision.");
            }
            ...
            lock (sync)
            {
                ...
                {
                    throw new InvalidOperationException(
                        "The committed retirement decision does not match the expected motion source evidence.");
                }
                ...
                {
                    throw new InvalidOperationException(
                        "Motion recovery changed after operator confirmation; retirement was not applied.");
                }
            ...
            CaptureActiveRetirementEvidenceCore()
        {
            ...
            {
                throw new InvalidOperationException(
                    "No active motion recovery record exists for operator retirement.");
            }
            ...
            if (!RecordsEqual(currentRecord, diskRecord))
            {
                throw new InvalidDataException(
                    "Motion recovery memory state does not match the exact durable source bytes.");
            }
            ...
            return new RecoveryJournalSourceEvidence(
                ...
                diskRecord.TargetKind.ToString(),
                diskRecord.TargetName,
                diskRecord.TargetReference,
                diskRecord.Operation,
                "TargetKind=" + diskRecord.TargetKind,
                originalBytes);
        }
        ...
        private byte[] ReadRetirementSourceBytes()
        {
                ...
                FileShare.Read))
            {
                ...
                {
                    throw new InvalidDataException(
                        "Motion recovery source length is invalid.");
                }
                ...
                while (offset < bytes.Length)
                {
                    ...
                    {
                        throw new InvalidDataException(
                            "Motion recovery source is truncated.");
                    }
                    offset += read;
                }
                return bytes;
        ...
        private static bool RecordsEqual(
            MotionUncertaintyRecord left,
            MotionUncertaintyRecord right)
        {
            return left != null
                && right != null
