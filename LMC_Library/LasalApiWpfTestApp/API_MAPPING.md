...
| Set Axis Position | Start `0x7D12`, outcome `0x7D14`, retire `0x7D1A` | SDK는 diagnostics identity+4 x U32 intent one-shot mutation, original BootId recovery key와 fresh current BootId를 분리한 future durable query/retirement 계약을 제공한다. current backing은 ordinary volatile `VAR_GLOBAL`; Store/ownership FALSE, bit 3/5/7 OFF, max-jump 0, native call 0이며 valid request도 detail 24다. WPF durable journal core는 MainWindow dispatch와 연결하지 않는다. durable backend/tombstone, unified ownership, task/core/priority, authoritative max-jump 및 PLC/hardware proof 전에는 control을 노출하지 않는다. |
| LMC Home | Start `0x7D13`, outcome `0x7D18`, retire `0x7D19` | v1 CurrentPositionZero wire를 보존하고 v2 typed parameter/wire를 추가했다. `Direct`와 LASAL `MoveReference` 기반 `AbsoluteSwitch`/`LimitSwitch`/`ReferencePulse` source 경로가 활성화되어 있다. WPF는 Home Position, Reference Velocity 1/2, Reference Acceleration, Reference Position Window를 편집하고 Start ACK 뒤 read-only outcome monitor와 exact terminal retirement를 수행한다. 기존 `Velocity/Acceleration/TorqueLimit/DistanceLimit` wire slot은 각각 VRef1/Accel/VRef2/PositionWindow로 해석한다. `Block`은 fail-closed다. exact rejected Start의 새 reservation은 rollback하며 오래된 Home record나 uncertain Start는 자동 삭제/replay하지 않는다. 빌드/실축 qualification은 대기 상태다. |
| DS402 Home | Start `0x7D15`, outcome `0x7D16`, retire `0x7D17` | WPF는 `PrepareLMC_HomeDS402`, `LMC_HomeDS402Async`, `ReadDs402HomeOutcomeAsync`, `RetireDs402HomeOutcomeAsync` public API를 직접 호출한다. Method `1..14`, `17..30`, `33`, `34`, `37`, Home Offset, Home Velocity1/2와 Home Acceleration을 편집할 수 있다. `PrepareDs402Home`과 `Ds402Home[Async]`는 호환용 이름이다. Admin bit 6과 runtime gate는 ON이며 Start ACK와 terminal outcome/retirement를 구분한다. |
