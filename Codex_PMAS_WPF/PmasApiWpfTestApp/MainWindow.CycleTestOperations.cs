    ...
    public partial class MainWindow
    {
        ...
        private async void ButtonStartCycleTest3_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                Context.Log(string.Format(
                    CultureInfo.InvariantCulture,
                    "CycleTest3 started(status-word in-position): base={0}, forward={1}, distanceMm={2}, vel={3}, acc={4}, dec={5}, jerk={6}, cycles={7}, warmupCycles={8}, timeoutMs={9}, pollMs={10}, stable={11}, inPosMask=0x{12:X4}, highPriority={13}, highPrecisionWait={14}, timer1ms={15}",
                    options.BasePosition,
                    options.ForwardPosition,
                    options.MoveDistanceMm,
                    options.Velocity,
        ...
        private CycleTestMetrics ExecuteCycleTest3(CycleTestOptions options, CancellationToken token)
        {
            ...
            else
            {
                Context.Log(string.Format(
                    CultureInfo.InvariantCulture,
                    "CycleTest3 warm-up started: cycles={0} (excluded from recording)",
                    warmupCycles));
            }
            ...
            try
            {
                    ...
                    : null)
                {
                    for (int cycleIndex = 1; cycleIndex <= totalCycles; cycleIndex++)
                    {
                        var isWarmup = cycleIndex <= warmupCycles;
                        var recordedCycleIndex = isWarmup ? cycleIndex : cycleIndex - warmupCycles;
                        var phaseLabel = isWarmup ? "Warm-up" : "Cycle3";
                        ...
                        if (!measurementStarted && !isWarmup)
                        {
                            ...
                            totalStopwatch.Restart();
                            measurementStarted = true;
                            Context.Log("CycleTest3 measurement recording started after warm-up.");
                        }
                        ...
                        try
                        {
                            ...
                            if (!forwardResult.Success)
                            {
                                ...
                                Context.Log(string.Format(
                                    CultureInfo.InvariantCulture,
                                    "{0} {1}: forward timeout (mask=0x{2:X4})",
                                    phaseLabel,
                                    recordedCycleIndex,
                                    options.InPositionStatusWordMask));
                                if (options.StopOnTimeout)
                            ...
                            if (!returnResult.Success)
                            {
                                ...
                                Context.Log(string.Format(
                                    CultureInfo.InvariantCulture,
                                    "{0} {1}: return timeout (mask=0x{2:X4})",
                                    phaseLabel,
                                    recordedCycleIndex,
                                    options.InPositionStatusWordMask));
                                if (options.StopOnTimeout)
        ...
        private int IssueCycleMove(double targetPosition, CycleTestOptions options)
        {
            ...
            if (result != 0)
            {
                Context.Log(string.Format(
                    CultureInfo.InvariantCulture,
                    "MoveAbsoluteEx returned non-zero code: {0}",
                    result));
            }
