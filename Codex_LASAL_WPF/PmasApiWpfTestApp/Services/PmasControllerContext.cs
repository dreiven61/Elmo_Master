...
namespace PmasApiWpfTestApp.Services
{
    public sealed class PmasControllerContext : INotifyPropertyChanged
    {
        ...
        public void EnsureAxis()
        {
            ...
            if (SingleAxis == null)
            {
                var fallbackAxisName = string.IsNullOrWhiteSpace(AxisName) ? "Axis1" : AxisName.Trim();
                SingleAxis = new MMCSingleAxis(fallbackAxisName, Handle);
                AxisName = fallbackAxisName;
                Log(string.Format(
                    CultureInfo.InvariantCulture,
                    "Axis load step is skipped. Fallback axis is auto-created: {0}",
                    fallbackAxisName));
            }
        }
